...
describe('ProperLabels geojson_diff_bin handling', () => {
  it('decodes add and compacted update diffs and applies updateData + ack', async () => {
    ...
    const addFeatures = [
      { id: 'u1', geometry: { type: 'Point', coordinates: [5, 6] }, properties: {} },
    ];
    const {
      meta: addMeta,
      keys: addKeys,
      propsBuffer: addPropsBuf,
      coordsArray: addCoords,
    } = encodeFeaturesBinary(addFeatures);
    ...
    const msg = {
      ...
      add: {
        meta: addMeta,
        keys: addKeys,
        propsBuf: addPropsBuf ? addPropsBuf.buffer : null,
        coords: addCoords.buffer,
      },
      updateDiffsMeta,
      updateKeys: upKeys,
