...
describe('worker update flow', () => {
  it('commits pending diff and responds with a diff when a feature updates', async () => {
    ...
    globalThis.__test_setPendingDiff({
      addList: [ { feature: { id: 'u1', geometry: { type: 'Point', coordinates: [1,1] }, properties: {} }, rawHash: 1, geomHash: 1 } ],
      updateList: [],
      removeList: []
    });
    ...
    const updated = [{ id: 'u1', geometry: { type: 'Point', coordinates: [2,3] }, properties: {} }];
    const { meta, keys, propsBuffer, coordsArray } = encodeFeaturesBinary(updated);

    globalThis.postMessage.mockClear();
    globalThis.onmessage({ data: { type: 'features_bin', meta, keys, propsBuf: propsBuffer.buffer, coords: coordsArray.buffer } });
    ...
    expect(first && (first.type === 'geojson_diff_bin' || first.type === 'geojson_diff')).toBe(true);
  });
});
