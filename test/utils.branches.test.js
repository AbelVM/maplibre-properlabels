...
describe('encode/decode branch coverage targets', () => {
  it('pushPoint accepts object {x,y}', () => {
    const features = [{ id: 'o', geometry: { type: 'Point', coordinates: { x: 5, y: -10 } }, properties: { a: 1 } }];
  ...
  it('propsBuffer smaller than needed triggers allocation and decodes correctly', () => {
    const features = [{ id: 'p', geometry: { type: 'Point', coordinates: [0, 0] }, properties: { big: 'abcdefghijklmnopqrstuvwxyz' } }];
    const small = new Uint8Array(2);
    const { meta, keys, propsBuffer, coordsArray } = encodeFeaturesBinary(features, { propsBuffer: small });
    const out = decodeFeaturesBinary(meta, propsBuffer, coordsArray, keys);
  ...
  it('coordsBuffer provided but too small falls back to new Float32Array', () => {
    const features = [{ id: 'ls', geometry: { type: 'LineString', coordinates: [[1, 1], [2, 2], [3, 3]] }, properties: {} }];
    const smallCoords = new Float32Array(1);
    const { meta, keys, propsBuffer, coordsArray } = encodeFeaturesBinary(features, { coordsBuffer: smallCoords });
    const out = decodeFeaturesBinary(meta, propsBuffer, coordsArray, keys);
  ...
  it('useSharedKeyTable populates sharedKeyTable and reset clears it', () => {
    ...
    const features = [{ id: 1, geometry: { type: 'Point', coordinates: [0, 0] }, properties: { x: 1, y: 2 } }];
  ...
  it('decode ignores invalid JSON property bytes', () => {
    const meta = [{ id: 'inv', type: 'Point', coordsOffset: 0, coordsLength: 2, props: [[0, 0, 3]] }];
...
describe('hashing and equality helpers', () => {
  it('computeGeometryHash differs by geometry type and coordinates', () => {
    const a = computeGeometryHash({ type: 'LineString', coordinates: [[0, 0], [1, 1]] });
    const b = computeGeometryHash({ type: 'LineString', coordinates: [[0, 0], [2, 2]] });
    expect(a).not.toBe(b);
  ...
  it('geometryEquals handles nulls, mismatched types and numeric epsilon', () => {
    ...
    expect(geometryEquals({ type: 'Point', coordinates: [0, 0] }, { type: 'Point', coordinates: [0.0000001, 0] })).toBe(true);
  });
});
