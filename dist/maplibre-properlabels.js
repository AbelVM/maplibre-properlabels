(function(v,x){typeof exports=="object"&&typeof module<"u"?module.exports=x():typeof define=="function"&&define.amd?define(x):(v=typeof globalThis<"u"?globalThis:v||self,v.ProperLabels=x())})(this,(function(){"use strict";let v,x;function H(){return v!==void 0?v===!1?null:v:typeof TextEncoder<"u"?(v=new TextEncoder,v):typeof Buffer<"u"&&typeof Buffer.from=="function"?(v={encode:h=>new Uint8Array(Buffer.from(h))},v):(v=!1,null)}function K(){return x!==void 0?x===!1?null:x:typeof TextDecoder<"u"?(x=new TextDecoder,x):typeof Buffer<"u"&&typeof Buffer.from=="function"?(x={decode:h=>Buffer.from(h).toString("utf8")},x):(x=!1,null)}const I=h=>{if(h instanceof Uint8Array)return h;if(ArrayBuffer.isView(h))return new Uint8Array(h.buffer,h.byteOffset,h.byteLength);if(h instanceof ArrayBuffer)return new Uint8Array(h);const e=JSON.stringify(h),n=H();if(n&&typeof n.encode=="function")return n.encode(e);throw new Error("No TextEncoder or Buffer available to encode object")},j=h=>{let e;if(h instanceof Uint8Array)e=h;else if(ArrayBuffer.isView(h))e=new Uint8Array(h.buffer,h.byteOffset,h.byteLength);else if(h instanceof ArrayBuffer)e=new Uint8Array(h);else if(typeof Buffer<"u"&&typeof Buffer.isBuffer=="function"&&Buffer.isBuffer(h))e=new Uint8Array(h);else throw new TypeError("Unsupported input to u82o, expected ArrayBuffer/TypedArray/Buffer");const n=K();if(n&&typeof n.decode=="function")return JSON.parse(n.decode(e));if(typeof TextDecoder<"u")return JSON.parse(new TextDecoder().decode(e));throw new Error("No TextDecoder or Buffer available to decode object")};class E{constructor({maxEntries:e=1/0,maxWeight:n=1/0,weightFn:t=()=>1,defaultTTL:r=6e4,maxPoolSize:i=1e3,rejectOversized:a=!1,onEvict:o=null,onExpire:s=null,initialPoolSize:l=0,maxCleanupPerTick:c=100,eagerCleanupOnRead:u=!1}={}){this.maxEntries=e,this.maxWeight=n,this.weightFn=t,this.defaultTTL=r,this.maxPoolSize=i,this.rejectOversized=!!a,this.onEvict=typeof o=="function"?o:null,this.onExpire=typeof s=="function"?s:null,this.maxCleanupPerTick=Number.isFinite(+c)?Math.max(1,+c):100,this.eagerCleanupOnRead=!!u,this.map=new Map,this.head=null,this.tail=null,this.pool=[];for(let f=0;f<Math.min(l||0,this.maxPoolSize);f++)this.pool.push({key:null,value:null,weight:0,expiresAt:0,prev:null,next:null});this.currentWeight=0,this.hits=0,this.misses=0,this.evictions=0,this.rejected=0,this.expirations=0,this._cleanupTimer=null,this._cleanupRunning=!1,this._cleanupParams=null,this._cleanupCursor=null,this._cleanupCursorValid=!1,this._inflightPromises=new Map}_allocNode(e,n,t,r){const i=this.pool.pop()||{key:null,value:null,weight:0,expiresAt:0,prev:null,next:null};return i.key=e,i.value=n,i.weight=t||0,i.expiresAt=r||0,i.prev=null,i.next=null,i}_freeNode(e){e.key=null,e.value=null,e.weight=0,e.expiresAt=0,e.prev=null,e.next=null,this.pool.length<this.maxPoolSize&&this.pool.push(e)}_removeExpiredNode(e,n,t=!1){if(!e||!e.expiresAt||e.expiresAt>n)return!1;const r=e.key,i=e.value,a=e.next;this.map.delete(r),this.currentWeight-=e.weight||0,this._cleanupCursor===e&&(this._cleanupCursor=a),this._cleanupCursorValid=!!this._cleanupCursor,this._remove(e);try{this.onExpire&&this.onExpire(r,i)}catch{}return this._freeNode(e),t&&this.misses++,this.expirations++,!0}_fetchValidNode(e,{ignoreExpiry:n=!1,countMiss:t=!1,allowExpired:r=!1}={}){const i=this.map.get(e);return i?!n&&i.expiresAt&&i.expiresAt<=Date.now()?r?i:(this._removeExpiredNode(i,Date.now(),t),null):i:(t&&this.misses++,null)}_refreshStaleEntry(e,n,{ttl:t=void 0,weight:r=void 0}={}){if(this._inflightPromises.has(e))return;let i;try{i=Promise.resolve().then(()=>n())}catch{return}const a=i.then(o=>{try{this.set(e,o,{ttl:t,weight:r})}catch{}return this._inflightPromises.delete(e),o},o=>{this._inflightPromises.delete(e)});this._inflightPromises.set(e,a)}_append(e){if(!this.tail){this.head=this.tail=e;return}e.prev=this.tail,e.next=null,this.tail.next=e,this.tail=e}_remove(e){const n=e.prev,t=e.next;n?n.next=t:this.head=t,t?t.prev=n:this.tail=n,e.prev=e.next=null}_moveToTail(e){this.tail!==e&&(this._remove(e),this._append(e))}_evictIfNeeded(){for(;this.map.size>this.maxEntries||this.currentWeight>this.maxWeight;){const e=this.head;if(!e)break;const n=e.next,t=e.key,r=e.value;this._cleanupCursor===e&&(this._cleanupCursor=n),this._cleanupCursorValid=!!this._cleanupCursor,this._remove(e),this.map.delete(t),this.currentWeight-=e.weight||0,this.evictions++;try{this.onEvict&&this.onEvict(t,r,"evicted")}catch{}this._freeNode(e)}}set(e,n,{ttl:t=this.defaultTTL,weight:r=null}={}){const i=Date.now(),a=t==null||t===1/0?0:i+t;let o;if(r!=null)o=r;else{try{o=this.weightFn(n)}catch{o=0}o==null&&(o=0)}const s=Number.isFinite(+o)?Math.max(0,+o):0;if(this.rejectOversized&&Number.isFinite(this.maxWeight)&&s>this.maxWeight){this.rejected++;try{this.onEvict&&this.onEvict(e,n,"rejected-oversized")}catch{}return!1}if(this.map.has(e)){const l=this.map.get(e);this.currentWeight-=l.weight||0,l.value=n,l.weight=s,l.expiresAt=a,this.currentWeight+=l.weight||0,this._moveToTail(l)}else{const l=this._allocNode(e,n,s,a);this.map.set(e,l),this._append(l),this.currentWeight+=l.weight||0,this._evictIfNeeded()}return this}get(e){const n=this._fetchValidNode(e,{countMiss:!0});if(n)return this._moveToTail(n),this.hits++,n.value}peek(e){const n=this._fetchValidNode(e);return n?n.value:void 0}has(e,{ignoreExpiry:n=!1}={}){return!!this._fetchValidNode(e,{ignoreExpiry:n})}getOrSet(e,n,{ttl:t=void 0,weight:r=void 0,staleWhileRevalidate:i=!1}={}){const a=Date.now(),o=this._fetchValidNode(e,{countMiss:!1,allowExpired:i});if(o)if(o.expiresAt&&o.expiresAt<=a){if(typeof n=="function")return this._moveToTail(o),this.hits++,this._refreshStaleEntry(e,n,{ttl:t,weight:r}),o.value;this._removeExpiredNode(o,a,!0)}else return this._moveToTail(o),this.hits++,o.value;else this.misses++;if(typeof n=="function"){const s=n();return s&&typeof s.then=="function"?s.then(l=>{try{this.set(e,l,{ttl:t,weight:r})}catch{}return l}):(this.set(e,s,{ttl:t,weight:r}),s)}return this.set(e,n,{ttl:t,weight:r}),n}setMany(e,{ttl:n=void 0,weight:t=void 0}={}){const r=Date.now(),i=n==null||n===1/0?0:r+n;for(const a of e){if(!a)continue;const[o,s]=a;let l;if(t!=null)l=t;else{try{l=this.weightFn(s)}catch{l=0}l==null&&(l=0)}const c=Number.isFinite(+l)?Math.max(0,+l):0;if(this.map.has(o)){const u=this.map.get(o);this.currentWeight-=u.weight||0,u.value=s,u.weight=c,u.expiresAt=i,this.currentWeight+=u.weight||0,this._moveToTail(u)}else{const u=this._allocNode(o,s,c,i);this.map.set(o,u),this._append(u),this.currentWeight+=u.weight||0}}return this._evictIfNeeded(),this}getMany(e,{ignoreExpiry:n=!1}={}){const t=new Map;for(const r of e){const i=this._fetchValidNode(r,{ignoreExpiry:n,countMiss:!0});i&&(this._moveToTail(i),this.hits++,t.set(r,i.value))}return t}touch(e,n=void 0){const t=this._fetchValidNode(e);if(!t)return!1;const r=Date.now();return n!==void 0&&(t.expiresAt=n==null||n===1/0?0:r+n),this._moveToTail(t),!0}getOrSetAsync(e,n,{ttl:t=void 0,weight:r=void 0,staleWhileRevalidate:i=!1}={}){if(typeof n!="function")return Promise.resolve(this.getOrSet(e,n,{ttl:t,weight:r}));const a=Date.now(),o=this.map.get(e);if(o)if(o.expiresAt&&o.expiresAt<=a){if(i)return this._moveToTail(o),this.hits++,this._refreshStaleEntry(e,n,{ttl:t,weight:r}),Promise.resolve(o.value);this._removeExpiredNode(o,a,!1)}else return this._moveToTail(o),this.hits++,Promise.resolve(o.value);if(this._inflightPromises.has(e))return this._inflightPromises.get(e);this.misses++;let s;try{s=Promise.resolve().then(()=>n())}catch(c){return Promise.reject(c)}const l=s.then(c=>{try{this.set(e,c,{ttl:t,weight:r})}catch{}return this._inflightPromises.delete(e),c},c=>{throw this._inflightPromises.delete(e),c});return this._inflightPromises.set(e,l),l}hasEqual(e,n,{ignoreExpiry:t=!1,seen:r=void 0}={}){const i=this._fetchValidNode(e,{ignoreExpiry:t});if(!i)return!1;const a=i.value;return a===n?!0:typeof a!=="object"||a===null||typeof n!=="object"||n===null?a===n:M(a,n,r)}hasEqualWithSeen(e,n,t,{ignoreExpiry:r=!1}={}){return this.hasEqual(e,n,{ignoreExpiry:r,seen:t})}delete(e){const n=this.map.get(e);if(!n)return!1;const t=n.next;this.map.delete(e),this.currentWeight-=n.weight||0,this._cleanupCursor===n&&(this._cleanupCursor=t),this._cleanupCursorValid=!!this._cleanupCursor,this._remove(n);try{this.onEvict&&this.onEvict(n.key,n.value,"deleted")}catch{}return this._freeNode(n),!0}clear(){for(let e=this.head;e;){const n=e.next;this._freeNode(e),e=n}this.head=this.tail=null,this.map.clear(),this.currentWeight=0,this._cleanupCursor=null,this._cleanupCursorValid=!1}cleanupExpired(){return this.cleanupExpiredUpTo()}cleanupExpiredUpTo(e=1/0){const n=Date.now();let t=0,r=this._cleanupCursor&&this._cleanupCursorValid?this._cleanupCursor:this.head;for(;r&&t<e;){const i=r.next;if(r.expiresAt&&r.expiresAt<=n){const a=r.key,o=r.value;this.map.delete(a),this.currentWeight-=r.weight||0,this._cleanupCursor===r&&(this._cleanupCursor=i),this._cleanupCursorValid=!!this._cleanupCursor,this._remove(r);try{this.onExpire&&this.onExpire(a,o)}catch{}this._freeNode(r),this.expirations++}r=i,t++}return this._cleanupCursor=r||this.head,this._cleanupCursorValid=!!this._cleanupCursor,t}startCleanup(e={}){let n,t;typeof e=="number"?(n=e,t=this.maxCleanupPerTick):(n=Number.isFinite(+e.interval)?+e.interval:Math.max(1e3,Math.min(this.defaultTTL||6e4,6e4)),t=Number.isFinite(+e.maxCleanupPerTick)?Math.max(1,+e.maxCleanupPerTick):this.maxCleanupPerTick),this.stopCleanup(),this._cleanupParams={interval:n,maxCleanupPerTick:t},this._cleanupTimer=setTimeout(()=>this._cleanupTick(),n)}stopCleanup(){this._cleanupTimer&&(clearTimeout(this._cleanupTimer),this._cleanupTimer=null),this._cleanupRunning=!1,this._cleanupParams=null}[Symbol.dispose](){try{this.stopCleanup()}catch{}try{this.clear()}catch{}}async[Symbol.asyncDispose](){try{this.stopCleanup()}catch{}try{this.clear()}catch{}}_cleanupTick(){if(this._cleanupTimer!=null){if(this._cleanupRunning){this._cleanupTimer=setTimeout(()=>this._cleanupTick(),this._cleanupParams.interval);return}this._cleanupRunning=!0;try{this.cleanupExpiredUpTo(this._cleanupParams.maxCleanupPerTick)}finally{this._cleanupRunning=!1}this._cleanupTimer=setTimeout(()=>this._cleanupTick(),this._cleanupParams.interval)}}get size(){return this.map.size}get hitRate(){const e=(this.hits||0)+(this.misses||0);return e?this.hits/e:0}stats(){return{size:this.size,weight:this.currentWeight,hits:this.hits,misses:this.misses,evictions:this.evictions,expirations:this.expirations,rejected:this.rejected,poolSize:this.pool.length}}resize({maxEntries:e,maxWeight:n}={}){Number.isFinite(+e)&&(this.maxEntries=Math.max(0,+e)),Number.isFinite(+n)&&(this.maxWeight=Math.max(0,+n)),this._evictIfNeeded()}*entries(e="MRU"){if(e==="MRU")for(let n=this.tail;n;n=n.prev)yield[n.key,n.value];else for(let n=this.head;n;n=n.next)yield[n.key,n.value]}[Symbol.iterator](){return this.entries("MRU")}*keys(e="MRU"){for(const[n]of this.entries(e))yield n}*values(e="MRU"){for(const[,n]of this.entries(e))yield n}}function M(h,e,n=void 0){if(h===e)return!0;if(h==null||e==null||typeof h!=="object"||typeof e!=="object")return h===e;n||(n=new WeakMap);let i=n.get(h);if(i&&i.has(e))return!0;if(i||(i=new WeakSet,n.set(h,i)),i.add(e),Object.getPrototypeOf(h)!==Object.getPrototypeOf(e))return!1;if(typeof Uint8Array<"u"&&h instanceof Uint8Array){if(!(e instanceof Uint8Array)||h.length!==e.length)return!1;for(let s=0;s<h.length;s++)if(h[s]!==e[s])return!1;return!0}if(Array.isArray(h)){if(!Array.isArray(e)||h.length!==e.length)return!1;for(let s=0;s<h.length;s++)if(!M(h[s],e[s],n))return!1;return!0}if(ArrayBuffer.isView(h)){if(!ArrayBuffer.isView(e)||h.byteLength!==e.byteLength)return!1;const s=new Uint8Array(h.buffer,h.byteOffset||0,h.byteLength),l=new Uint8Array(e.buffer,e.byteOffset||0,e.byteLength);for(let c=0;c<s.length;c++)if(s[c]!==l[c])return!1;return!0}if(h instanceof ArrayBuffer){if(!(e instanceof ArrayBuffer)||h.byteLength!==e.byteLength)return!1;const s=new Uint8Array(h),l=new Uint8Array(e);for(let c=0;c<s.length;c++)if(s[c]!==l[c])return!1;return!0}if(h instanceof Date)return e instanceof Date?h.getTime()===e.getTime():!1;if(h instanceof RegExp)return e instanceof RegExp?h.toString()===e.toString():!1;if(h instanceof Map){if(!(e instanceof Map)||h.size!==e.size)return!1;for(const[s,l]of h)if(!e.has(s)||!M(l,e.get(s),n))return!1;return!0}if(h instanceof Set){if(!(e instanceof Set)||h.size!==e.size)return!1;let s=!0;for(const l of h)if(l!==null&&typeof l=="object"){s=!1;break}if(s){for(const l of h)if(!e.has(l))return!1;return!0}for(const l of h){let c=!1;for(const u of e)if(M(l,u,n)){c=!0;break}if(!c)return!1}return!0}const a=Object.keys(h),o=Object.keys(e);if(a.length!==o.length)return!1;for(let s=0;s<a.length;s++){const l=a[s];if(!Object.prototype.hasOwnProperty.call(e,l)||!M(h[l],e[l],n))return!1}return!0}class T{constructor(e,n={}){const{keyResolver:t=(...o)=>JSON.stringify(o),cacheOptions:r={},ttl:i,weight:a}=n;if(this.keyResolver=typeof t=="function"?t:(...o)=>JSON.stringify(o),this.cache=new E(r),this._inflight=new Map,this._defaultMemoizeOptions={},i!==void 0&&(this._defaultMemoizeOptions.ttl=i),a!==void 0&&(this._defaultMemoizeOptions.weight=a),typeof e=="function"){const o=this._memoize(e,this._defaultMemoizeOptions);o.get=(...s)=>this.get(...s),o.has=(...s)=>this.has(...s),o.delete=(...s)=>this.delete(...s),o.clear=()=>this.clear(),o.stats=()=>this.stats(),o.cache=this.cache,o.original=e;try{Object.setPrototypeOf(o,T.prototype),o.constructor=T}catch{}return o}this.run=()=>{throw new TypeError("No function supplied to PowerMemoizer; call memoize(fn) or construct with a function.")},this._originalFn=null}_memoize(e,{ttl:n,weight:t}={}){if(typeof e!="function")throw new TypeError("fn must be a function");const r=this;return function(...a){const o=r.keyResolver(...a);if(r.cache.has(o))return r.cache.get(o);if(r._inflight.has(o))return r._inflight.get(o);const s=e(...a);if(s&&typeof s.then=="function"){const l=s.then(c=>{try{r.cache.set(o,c,{ttl:n,weight:t})}catch{}return r._inflight.delete(o),c},c=>{throw r._inflight.delete(o),c});return r._inflight.set(o,l),l}return r.cache.set(o,s,{ttl:n,weight:t}),s}}memoize(e,n={}){if(typeof e!="function")throw new TypeError("fn must be a function");const t=n&&(Object.prototype.hasOwnProperty.call(n,"ttl")||Object.prototype.hasOwnProperty.call(n,"weight"))?n:this._defaultMemoizeOptions,r=this._memoize(e,t);r.get=(...i)=>this.get(...i),r.has=(...i)=>this.has(...i),r.delete=(...i)=>this.delete(...i),r.clear=()=>this.clear(),r.stats=()=>this.stats(),r.cache=this.cache,r.original=e;try{Object.setPrototypeOf(r,T.prototype),r.constructor=T}catch{}return r}get(...e){return this.cache.get(this.keyResolver(...e))}has(...e){return this.cache.has(this.keyResolver(...e))}delete(...e){const n=this.keyResolver(...e);return this._inflight.has(n)&&this._inflight.delete(n),this.cache.delete(n)}clear(){this._inflight.clear(),this.cache.clear()}stats(){return this.cache.stats()}}let P=null;if(typeof process<"u"&&process.hrtime&&typeof process.hrtime.bigint=="function")try{const h=Number(process.hrtime.bigint()/1000000n);P=Date.now()-h}catch{P=null}const w=()=>{const h=Date.now();if(typeof performance<"u"&&typeof performance.now=="function"&&typeof performance.timeOrigin=="number")try{const e=performance.timeOrigin+performance.now();return Math.abs(e-h)<1e3?e:h}catch{}if(P!=null)try{const e=Number(process.hrtime.bigint()/1000000n)+P;return Math.abs(e-h)<1e3?e:h}catch{return h}return h};class L{constructor(e=16){const n=Math.max(2,Number(e)||16);for(this._capacity=1;this._capacity<n;)this._capacity<<=1;this._mask=this._capacity-1,this._buffer=new Array(this._capacity),this._head=0,this._tail=0,this._size=0}push(e){return this._size===this._capacity&&this._grow(),this._buffer[this._tail]=e,this._tail=this._tail+1&this._mask,this._size++,this._size}shift(){if(this._size===0)return;const e=this._buffer[this._head];return this._buffer[this._head]=void 0,this._head=this._head+1&this._mask,this._size--,e}peek(){return this._size===0?void 0:this._buffer[this._head]}clear(){if(this._size===0)return;let e=this._head;for(let n=0;n<this._size;n++)this._buffer[e]=void 0,e=e+1&this._mask;this._head=this._tail=0,this._size=0}get length(){return this._size}get capacity(){return this._capacity}get isEmpty(){return this._size===0}*[Symbol.iterator](){let e=this._head;for(let n=0;n<this._size;n++)yield this._buffer[e+n&this._mask]}values(){return this[Symbol.iterator]()}*keys(){for(let e=0;e<this._size;e++)yield e}*entries(){for(let e=0;e<this._size;e++)yield[e,this._buffer[this._head+e&this._mask]]}*drain(){for(;this._size>0;)yield this.shift()}toArray(){const e=new Array(this._size);for(let n=0;n<this._size;n++)e[n]=this._buffer[this._head+n&this._mask];return e}_grow(){const e=this._buffer,t=this._capacity<<1,r=new Array(t);for(let i=0;i<this._size;i++)r[i]=e[this._head+i&this._mask];this._buffer=r,this._capacity=t,this._mask=t-1,this._head=0,this._tail=this._size&this._mask}pushMany(e){if(!Array.isArray(e)||e.length===0)return this._size;const n=this._size+e.length;for(;this._capacity<n;)this._grow();const t=Math.min(e.length,this._capacity-this._tail);for(let i=0;i<t;i++)this._buffer[this._tail+i]=e[i];this._tail=this._tail+t&this._mask;let r=t;for(;r<e.length;){const i=Math.min(e.length-r,this._capacity-this._tail);for(let a=0;a<i;a++)this._buffer[this._tail+a]=e[r+a];this._tail=this._tail+i&this._mask,r+=i}return this._size=n,this._size}unshiftMany(e){if(!Array.isArray(e)||e.length===0)return this._size;const n=this._size+e.length;for(;this._capacity<n;)this._grow();let t=this._head-e.length&this._mask;for(let r=0;r<e.length;r++)this._buffer[t+r&this._mask]=e[r];return this._head=t,this._size=n,this._size}}function J(h,e="ERR_ITEM"){return!h||typeof h!="object"?{error:!0,code:e,message:h?String(h):void 0,stack:void 0}:{error:!0,code:h.code||e,message:h.message,stack:h.stack}}function F(h){return!h||!h.error?String(h):`${h.code||"ERR"}: ${h.message||""}`}const k=typeof globalThis<"u"&&globalThis&&globalThis.console?globalThis.console:typeof self<"u"&&self&&self.console?self.console:typeof window<"u"&&window&&window.console?window.console:typeof global<"u"&&global&&global.console?global.console:null;class B{constructor(e=0,n={}){this._debugLevel=0,this._counters=Object.create(null),this._format=n&&n.format||"text",this.name=n&&n.name||null,this._formatter=n&&typeof n.formatter=="function"?n.formatter:null,this._output=n&&typeof n.output=="function"?n.output:null,this.setDebugLevel(e)}setDebugLevel(e){let n=NaN;typeof e=="number"?n=e:typeof e=="string"||typeof e=="boolean"?n=Number(e):(e instanceof Number||e instanceof String||e instanceof Boolean)&&(n=Number(e.valueOf())),this._debugLevel=Number.isFinite(n)&&n>=0?Math.max(0,Math.min(3,Math.floor(n))):0}getDebugLevel(){return this._debugLevel}isDebugLevel(e=1){return Number(this._debugLevel)>=Number(e||1)}isDebug(){return this.isDebugLevel(1)}_resolveLogArgs(e){return e.map(n=>{if(typeof n=="function")try{return n()}catch(t){return t}return n})}_emit(e,n,t,r,i={}){if(!this.isDebugLevel(e))return;const a=this._resolveLogArgs(r),o=i.msgArray?a:a.length===1?a[0]:a;let s={level:t,msg:o,ts:w(),format:this._format};if(this.name&&(s.name=this.name),this._formatter)try{const l=this._formatter(s);if(l!=null){if(typeof l=="string"){if(this._output){try{this._output(l)}catch{}return}k&&typeof k[n]=="function"&&k[n](l);return}s=l}}catch{}if(this._output){try{this._output(s)}catch{}return}if(!(!k||typeof k[n]!="function"))if(this._format==="json")try{const l=typeof s=="string"?s:JSON.stringify(s);k[n](l)}catch{k[n](...Array.isArray(a)?a:[a])}else k[n](...a)}error(...e){const n=e.map(t=>{try{if(t&&t.error)return F(t);if(t instanceof Error||t&&typeof t=="object")return F(J(t))}catch{}return t});this._emit(1,"error","error",n)}warn(...e){this._emit(2,"warn","warn",e)}info(...e){this._emit(3,"info","info",e)}log(...e){this._emit(3,"log","log",e)}debug(...e){this._emit(3,"debug","debug",e)}table(...e){if(!this.isDebugLevel(3)||!k)return;if(this._format==="json"){this._emit(3,"log","table",e,{msgArray:!0});return}const n=this._resolveLogArgs(e);typeof k.table=="function"?k.table(...n):typeof k.log=="function"&&k.log(...n)}incrementCounter(e){if(!this.isDebug())return;const n=String(e||"");n&&(this._counters[n]=(this._counters[n]||0)+1)}getDebugCounters(){return Object.assign({},this._counters)}resetDebugCounters(){this._counters=Object.create(null)}}const Y=Symbol("PowerSubscriberSet.original");class S{constructor(e={}){const{weak:n=!1,maxListeners:t=0}=e||{};this._weak=!!n,this._maxListeners=Number.isFinite(Number(t))?Math.max(0,Math.floor(Number(t))):0,this._listeners=new Set,this._onceMap=new WeakMap,this._finalization=null,this._weak&&typeof WeakRef<"u"&&typeof FinalizationRegistry<"u"&&(this._finalization=new FinalizationRegistry(r=>{this._listeners.delete(r.ref)}))}get size(){return this._cleanup(),this._listeners.size}add(e){if(typeof e!="function"){if(!this._weak||!e||typeof e.deref!="function")throw new TypeError("listener must be a function");if(this._maxListeners>0&&this.size+1>this._maxListeners)throw new Error(`PowerSubscriberSet: adding listener exceeds maxListeners (${this._maxListeners})`);return this._listeners.add(e),()=>this.delete(e)}if(this._maxListeners>0&&this.size+1>this._maxListeners)throw new Error(`PowerSubscriberSet: adding listener exceeds maxListeners (${this._maxListeners})`);const n=this._makeEntry(e);return this._listeners.add(n),()=>this.delete(e)}addOnce(e){if(typeof e!="function")throw new TypeError("listener must be a function");const n=(...r)=>{try{e(...r)}finally{this.delete(e)}};try{n[Y]=e}catch{}if(this._onceMap.set(e,n),this._maxListeners>0&&this.size+1>this._maxListeners)throw new Error(`PowerSubscriberSet: adding listener exceeds maxListeners (${this._maxListeners})`);const t=this._makeEntry(n);return this._listeners.add(t),()=>this.delete(e)}delete(e){let n=e;const t=this._onceMap.get(e);t&&(n=t,this._onceMap.delete(e));for(const r of this._listeners){const i=this._deref(r);if(!i){this._listeners.delete(r);continue}if(i===n)return this._listeners.delete(r),this._finalization&&typeof r.deref=="function"&&this._finalization.unregister(r),!0}return!1}forEach(e){for(const n of this._listeners){const t=this._deref(n);if(!t){this._listeners.delete(n);continue}e(t)}}clear(){this._listeners.clear(),this._onceMap=new WeakMap}values(){this._cleanup();const e=[];for(const n of this._listeners){const t=this._deref(n);t&&e.push(t)}return e}[Symbol.iterator](){return this.values()[Symbol.iterator]()}_cleanup(){if(!(!this._weak||typeof WeakRef>"u"))for(const e of this._listeners)e&&typeof e.deref=="function"&&!e.deref()&&this._listeners.delete(e)}_makeEntry(e){if(this._weak&&typeof WeakRef<"u"){const n=new WeakRef(e);if(this._finalization)try{this._finalization.register(e,{ref:n},n)}catch{}return n}return e}_deref(e){return e&&typeof e.deref=="function"?e.deref():e}}function Z(h){if(h){if(typeof h.cleanup=="function"){try{h.cleanup()}catch{}return}if(typeof h._cleanup=="function"){try{h._cleanup()}catch{}return}if(typeof h[Symbol.iterator]=="function"&&typeof h.delete=="function")for(const e of h)(e&&typeof e.deref=="function"?e.deref():e)||h.delete(e)}}class q{constructor(e={}){this._listeners=new Map,this._maxListeners=Number.isFinite(Number(e.maxListeners))?Math.max(0,Number(e.maxListeners)):0,this._weak=!!e.weak,this._fr=null,this._finalizationRefs=new WeakMap}_ensureFinalizationRegistry(){return!this._weak||typeof FinalizationRegistry>"u"?null:this._fr?this._fr:(this._fr=new FinalizationRegistry(e=>{try{const{event:n,ref:t}=e,r=this._listeners.get(n);r&&typeof r.delete=="function"&&r.delete(t.deref?t.deref():t)}catch{}}),this._fr)}cleanup(){if(this._weak)for(const[e,n]of this._listeners)Z(n),n.size===0&&this._listeners.delete(e)}_getBucket(e){let n=this._listeners.get(e);if(!n)return null;if(n instanceof S)return n;if(n&&typeof n[Symbol.iterator]=="function"){const t=new S({maxListeners:this._maxListeners,weak:this._weak});for(const r of n){const i=r&&typeof r.deref=="function"?r.deref():r;i&&t.add(i)}return this._listeners.set(e,t),t}return null}_registerWeakListener(e,n){const t=this._ensureFinalizationRegistry();if(!t||typeof WeakRef>"u")return null;const r=new WeakRef(e);try{t.register(e,{event:n,ref:r},r),this._finalizationRefs.set(e,r)}catch{return null}return r}_unregisterWeakListener(e){if(!this._fr||!this._finalizationRefs.has(e))return;const n=this._finalizationRefs.get(e);try{this._fr.unregister(n)}catch{}this._finalizationRefs.delete(e)}on(e,n){if(typeof n!="function")throw new TypeError("listener must be a function");let t=this._getBucket(e);t||(t=new S({maxListeners:this._maxListeners,weak:this._weak}),this._listeners.set(e,t));const r=t.add(n);return this._registerWeakListener(n,e)?()=>{r(),this._unregisterWeakListener(n)}:r}once(e,n){if(typeof n!="function")throw new TypeError("listener must be a function");let t=this._getBucket(e);t||(t=new S({maxListeners:this._maxListeners,weak:this._weak}),this._listeners.set(e,t));const r=t.addOnce(n);return this._registerWeakListener(n,e)?()=>{r(),this._unregisterWeakListener(n)}:r}off(e,n){const t=this._getBucket(e);t&&(t.delete(n),this._unregisterWeakListener(n),t.size===0&&this._listeners.delete(e))}emit(e,n){const t=this._listeners.get(e);if(!t||t.size===0)return!1;if(t instanceof S){let i=!1;return t.forEach(a=>{i=!0;try{a(n)}catch{}}),t.size===0&&this._listeners.delete(e),i}const r=t.size>0;for(const i of t){const a=i&&typeof i.deref=="function"?i.deref():i;if(!a){t.delete(i);continue}try{a(n)}catch{}}return t.size===0&&this._listeners.delete(e),r}async emitAsync(e,n,{concurrency:t=1/0}={}){const r=this.listeners(e);if(r.length===0)return!1;const i=Number.isFinite(+t)&&+t>0?Math.max(1,Math.floor(+t)):1/0,a=async l=>{try{await l(n)}catch{}};if(!Number.isFinite(i)||i>=r.length)return await Promise.all(r.map(a)),!0;let o=0;const s=Array.from({length:i},async()=>{for(;o<r.length;){const l=r[o++];l&&await a(l)}});return await Promise.all(s),!0}listeners(e){const n=this._listeners.get(e);return n?n instanceof S?n.values():Array.from(n).map(t=>t&&typeof t.deref=="function"?t.deref():t).filter(Boolean):[]}clear(e){if(e===void 0){this._listeners.clear();return}this._listeners.delete(e)}}let X=class{constructor(e,n,t){this._underlying=e,this._logger=n,this._pool=t,this.onmessage=null,this.onerror=null,this.onmessageerror=null}postMessage(e,n){let t=e,r=n;if(e!==null&&typeof e=="object"&&!ArrayBuffer.isView(e)&&!(e instanceof ArrayBuffer))try{const a=this._pool._encodeForTransfer(e);if(!r||Array.isArray(r)&&r.length===0)r=[a.buffer];else{let o=!1;if(Array.isArray(r)){for(let s of r)if(s===a.buffer){o=!0;break}o||(r=[...r,a.buffer])}else if(r.length===0)r=[a.buffer];else{const s=[];for(let l of r)s.push(l),l===a.buffer&&(o=!0);o||s.push(a.buffer),r=s}}t=a}catch{r=n,t=e}if(!r&&(t instanceof Uint8Array||ArrayBuffer.isView(t))){const a=t.buffer;a&&a.byteLength>0?r=[a]:r=void 0}try{r&&r.length?this._underlying.postMessage(t,r):this._underlying.postMessage(t)}catch(a){throw this._logger.error(a,"Failed to postMessage to underlying worker"),a}}addEventListener(...e){return this._underlying.addEventListener(...e)}removeEventListener(...e){return this._underlying.removeEventListener(...e)}terminate(){typeof this._underlying.terminate=="function"&&this._underlying.terminate()}};class ee extends Error{constructor(e="PowerPool has been shut down"){super(e),this.name="PowerPoolShutdownError"}}class U{constructor(e,n={}){const t=typeof navigator<"u"&&navigator.hardwareConcurrency||2,{size:r=Math.min(t,2),minSize:i=2,maxSize:a=Math.max(r,t),workerOptions:o={},maxTasksPerWorker:s=1/0,idleTimeout:l=6e4,taskQueue:c=!0,queuePolicy:u="enqueue",lazy:f=!0}=n;this._workerSource=e,this._workerOptions=o,this._maxTasksPerWorker=s,this.minSize=Math.max(0,i),this.maxSize=Math.max(this.minSize,a),this.idleTimeout=Math.max(0,l),this.taskQueueEnabled=!!c,this._queuePolicy=["enqueue","drop-oldest","drop-newest","reject"].includes(u)?u:"enqueue",this._createdAt=w(),this._totalWorkersCreated=0,this._totalTasksCompleted=0,this._taskDurationsWelfordCount=0,this._taskDurationsWelfordMean=0,this._taskDurationsWelfordM2=0,this._taskDurationsMin=Number.POSITIVE_INFINITY,this._taskDurationsMax=Number.NEGATIVE_INFINITY,this._ewmaLatency=null,this._autoScale=null,this._autoScaleInterval=null,this._lastAutoScaleAt=null,this._terminatedWorkerTaskCountsTotal=0,this._terminatedWorkerTaskCountsCount=0,this.workers=[],this.queue=new L;const p={maxListeners:n&&(n.listenerMaxListeners??n.maxListeners),weak:n&&!!n.weakListeners};this._bus=new q(p),this._queueHighThreshold=Number.isFinite(Number(n&&n.queueHighThreshold))?Math.max(0,Math.floor(Number(n.queueHighThreshold))):1/0,this._queueHighCrossed=!1,this._onmessage=null,this._onerror=null,this._onidle=null,this._onresize=null,this._nextIndex=0,this._nextWorkerId=0,this._activeTasks=0,this._isIdle=!0;const m=n&&typeof n.debugLevel=="number"?n.debugLevel:1;this._logger=new B(m,{name:"powerPool"}),this._pendingResponses=new Map,this._underlyingToWorkerObj=new Map;const y=f?Math.min(this.minSize,this.maxSize):Math.min(Math.max(r,this.minSize),this.maxSize);for(let d=0;d<y;d++)this._addWorkerInstance();if(this._reaperInterval=setInterval(()=>this._reapIdleWorkers(),Math.max(1e3,Math.floor(this.idleTimeout/2))),this._encodeCache=new Map,this._encodeCacheLimit=Math.max(16,n&&n.encodeCacheLimit?n.encodeCacheLimit:64),this._encodeCacheByteLimit=Number.isFinite(n&&Number(n.encodeCacheByteLimit))?Math.max(0,Number(n.encodeCacheByteLimit)):1/0,this._encodeCacheBytes=0,n&&n.autoScale){const d=typeof n.autoScale=="object"?n.autoScale:{},g=Number.isFinite(Number(d.intervalMs))?Math.max(100,Math.floor(d.intervalMs)):1e3,b=Number.isFinite(Number(d.targetMs))?Math.max(1,Number(d.targetMs)):50,C=Number.isFinite(Number(d.alpha))?Math.max(0,Math.min(1,Number(d.alpha))):.2,A=Number.isFinite(Number(d.cooldownMs))?Math.max(0,Math.floor(d.cooldownMs)):5e3,z=Number.isFinite(Number(d.hysteresis))?Math.max(0,Math.min(1,Number(d.hysteresis))):.2,O=Number.isFinite(Number(d.stepUp))?Math.max(1,Math.floor(Number(d.stepUp))):1,_=Number.isFinite(Number(d.stepDown))?Math.max(1,Math.floor(Number(d.stepDown))):1,W=Number.isFinite(Number(d.backoffFactor))?Math.max(1,Number(d.backoffFactor)):1,le=Number.isFinite(Number(d.backoffMaxMultiplier))?Math.max(1,Number(d.backoffMaxMultiplier)):8,he=Number.isFinite(Number(d.backoffResetMs))?Math.max(0,Math.floor(Number(d.backoffResetMs))):A*4;this._autoScale={enabled:!0,intervalMs:g,targetMs:b,alpha:C,cooldownMs:A,hysteresis:z,stepUp:O,stepDown:_,backoffFactor:W,backoffMaxMultiplier:le,backoffResetMs:he},this._autoScaleBackoffMultiplier=1;try{this._autoScaleInterval=setInterval(()=>this._autoScaleTick(),g)}catch{}}}_debugLog(e,n){try{this&&this._logger&&typeof this._logger.debug=="function"&&(e?this._logger.debug(e,n||"swallowed error"):this._logger.debug(n||"swallowed error"))}catch{}}_ensureReaper(){try{this._reaperInterval||(this._reaperInterval=setInterval(()=>this._reapIdleWorkers(),Math.max(1e3,Math.floor(this.idleTimeout/2))))}catch{}}_clearLifecycleIntervals(){try{this._reaperInterval&&(clearInterval(this._reaperInterval),this._reaperInterval=null)}catch{}try{this._autoScaleInterval&&(clearInterval(this._autoScaleInterval),this._autoScaleInterval=null)}catch{}}shutdown(){this._clearLifecycleIntervals();try{for(const[e]of this._pendingResponses)try{this._cleanupPendingResponse(e,{rejectWith:new ee("pool:shutdown")})}catch(n){this._debugLog&&this._debugLog(n,"shutdown: cleanup pending response")}}catch(e){this._debugLog&&this._debugLog(e,"shutdown: iterate pending responses")}try{for(const e of this.workers)try{e.worker.terminate()}catch(n){this._debugLog&&this._debugLog(n,"shutdown: terminate worker")}}catch(e){this._debugLog&&this._debugLog(e,"shutdown: terminate workers loop")}try{this._underlyingToWorkerObj&&this._underlyingToWorkerObj.clear()}catch{}try{const e=this.workers.map(n=>n&&n.id).filter(n=>n!=null);e&&e.length&&this._bus.emit("pool:scale",{action:"remove",terminated:e,count:e.length})}catch(e){this._debugLog&&this._debugLog(e,"shutdown: pool scale emit error")}this.workers=[],this.queue=new L,this._activeTasks=0}_encodeForTransfer(e){try{const n=JSON.stringify(e);if(typeof n=="string"&&n.length>2048)return I(e);const t=this._encodeCache.get(n);if(t){try{this._encodeCache.delete(n),this._encodeCache.set(n,t)}catch{}return t}const r=I(e),i=r&&r.byteLength||0,a=()=>this._encodeCache.size>=this._encodeCacheLimit||this._encodeCacheByteLimit!==1/0&&this._encodeCacheBytes+i>this._encodeCacheByteLimit;for(;a();){const o=this._encodeCache.keys().next().value;if(!o)break;try{const s=this._encodeCache.get(o);s&&s.byteLength&&(this._encodeCacheBytes=Math.max(0,this._encodeCacheBytes-s.byteLength))}catch{}this._encodeCache.delete(o)}return this._encodeCache.set(n,r),r&&r.byteLength&&(this._encodeCacheBytes+=r.byteLength),r}catch{return I(e)}}prepareBuffer(e,n={}){const{clone:t=!0}=n,r=this._encodeForTransfer(e);return t?r.slice():r}prepareBuffers(e,n={}){if(!Array.isArray(e))throw new Error("prepareBuffers expects an array");const{clone:t=!0,zeroCopy:r=!1}=n,i=new Array(e.length);for(let a=0;a<e.length;a++){const o=e[a]&&typeof e[a]=="object"&&"message"in e[a]?e[a]:{message:e[a]},s=o.message,l=o.transfer;if(l){i[a]={message:s,transfer:l};continue}if(s!==null&&typeof s=="object"&&!ArrayBuffer.isView(s)&&!(s instanceof ArrayBuffer)){if(r){i[a]={message:s,transfer:void 0};continue}try{const u=this._encodeForTransfer(s);if(t){const f=u.slice();i[a]={message:f,transfer:[f.buffer]}}else i[a]={message:u,transfer:void 0};continue}catch{i[a]={message:s,transfer:void 0};continue}}if(s instanceof ArrayBuffer||ArrayBuffer.isView(s)){const u=s instanceof ArrayBuffer?s:s.buffer;i[a]={message:s,transfer:[u]};continue}i[a]={message:s,transfer:void 0}}return i}_prepareForTransfer(e,n,t){const r=t&&!!t.zeroCopy;if(e!==null&&typeof e=="object"&&!ArrayBuffer.isView(e)&&!(e instanceof ArrayBuffer)){if(r)return{message:e,transfer:n};try{const o=this._encodeForTransfer(e).slice();let s=n;if(!s||Array.isArray(s)&&s.length===0)s=[o.buffer];else if(Array.isArray(s)){let l=!1;for(const c of s)if(c===o.buffer){l=!0;break}l||(s=[...s,o.buffer])}else if(s.length===0)s=[o.buffer];else{const l=[];let c=!1;for(const u of s)l.push(u),u===o.buffer&&(c=!0);c||l.push(o.buffer),s=l}return{message:o,transfer:s}}catch{return{message:e,transfer:n}}}if(e instanceof Uint8Array||ArrayBuffer.isView(e)||e instanceof ArrayBuffer){const a=e instanceof ArrayBuffer?e:e.buffer;if(a&&a.byteLength===0)try{const o=e instanceof ArrayBuffer?e.slice(0):new Uint8Array(e);return{message:o,transfer:[o.buffer]}}catch{return{message:e,transfer:void 0}}return{message:e,transfer:[a]}}return{message:e,transfer:n}}_decrementActiveTasks(e=1){try{const n=Number.isFinite(Number(e))?Math.max(0,Math.floor(Number(e))):1;this._activeTasks=Math.max(0,this._activeTasks-n)}catch{this._activeTasks=0}}resize(e){let n=this.minSize,t=this.maxSize;if(e!=null&&typeof e=="object")Number.isFinite(e.minSize)&&(n=Math.max(0,Math.floor(e.minSize))),Number.isFinite(e.maxSize)&&(t=Math.max(n,Math.floor(e.maxSize)));else{const a=Number(e);if(!Number.isFinite(a))return;t=Math.max(n,Math.floor(a))}this.minSize=Math.max(0,n),this.maxSize=Math.max(this.minSize,t);let r=0;for(;this.workers.length<this.minSize&&this.workers.length<this.maxSize;)this._addWorkerInstance(),r++;const i=[];for(;this.workers.length>this.maxSize;){const a=this.workers.pop();if(a){this._decrementActiveTasks(a.tasks||0);try{a.worker.terminate()}catch{}this._deleteWorkerUnderlyingMapping(a),this._terminatedWorkerTaskCountsTotal+=a.completedTasks||0,this._terminatedWorkerTaskCountsCount+=1,i.push(a.id)}}if(i.length||r){const a={data:{type:"pool:resize",terminated:i,added:r,minSize:this.minSize,maxSize:this.maxSize}};if(this._onresize)try{this._onresize(a)}catch(o){this._logger.error(o,"Pool onresize handler error")}try{this._bus.emit("resize",a)}catch(o){this._logger.error(o,"pool resize listener error")}try{this._bus.emit("pool:scale",{added:r,terminated:i,minSize:this.minSize,maxSize:this.maxSize})}catch(o){this._logger.error(o,"pool scale resize listener error")}}this._updateIdleState()}_createWorkerInstance(){if(typeof this._workerSource=="function"){const e=this._workerSource;if(e.prototype===void 0)return e();try{return new e}catch(n){const t=String(n&&n.message);if(n instanceof TypeError&&/not a constructor|cannot be invoked without\s*'new'|Class constructor|not constructable/i.test(t))return e();throw n}}if(typeof this._workerSource=="string"){let e;try{e=new Function("try { return import.meta && import.meta.url } catch (e) { return undefined }")()}catch{e=void 0}if(!e&&typeof document<"u"){const n=document.currentScript;n&&n.src&&(e=n.src)}!e&&typeof location<"u"&&location.href&&(e=location.href);try{if(e)return new Worker(new URL(this._workerSource,e),this._workerOptions)}catch{}return new Worker(this._workerSource,this._workerOptions)}throw new Error("Invalid workerSource: expected Worker factory or relative path string")}_deleteWorkerUnderlyingMapping(e){try{const n=e&&e.worker&&e.worker._underlying;n&&this._underlyingToWorkerObj&&this._underlyingToWorkerObj.delete(n)}catch{}}_addWorkerInstance(e){e==null&&(e=this._nextWorkerId++);const n=this._createWorkerInstance(),t=new X(n,this._logger,this),r={id:e,worker:t,tasks:0,lastActive:w(),latencyEwma:null,_startTimes:new L};r.completedTasks=0,this.workers.push(r),this._totalWorkersCreated++;try{this._bus.emit("pool:scale",{action:"add",id:r.id,minSize:this.minSize,maxSize:this.maxSize})}catch(s){this._logger.error(s,"pool scale add listener error")}try{this._underlyingToWorkerObj.set(n,r)}catch{}t.onmessage=s=>{const l=w();r.tasks=Math.max(0,r.tasks-1),this._decrementActiveTasks(1),r.lastActive=l;try{const c=s&&s.data;if(c&&typeof c=="object"&&c.correlationId!=null){const u=String(c.correlationId),f=Object.prototype.hasOwnProperty.call(c,"response")?c.response:c;this._cleanupPendingResponse(u,{resolveWith:f})}}catch(c){this._debugLog&&this._debugLog(c,"worker.onmessage: resolve pending response")}try{const c=r._startTimes&&r._startTimes.length?r._startTimes.shift():null;let u=null;try{const f=s&&s.data;if(f&&typeof f.duration=="number"&&Number.isFinite(f.duration)?u=Math.max(0,Number(f.duration)):c!=null&&(u=Math.max(0,l-c)),u!=null){const p=this._autoScale&&this._autoScale.alpha||.2;r.latencyEwma==null?r.latencyEwma=u:r.latencyEwma=p*u+(1-p)*r.latencyEwma,this._ewmaLatency==null?this._ewmaLatency=u:this._ewmaLatency=p*u+(1-p)*this._ewmaLatency,this._totalTasksCompleted=(this._totalTasksCompleted||0)+1,r.completedTasks=(r.completedTasks||0)+1;const m=1,y=this._taskDurationsWelfordCount;this._taskDurationsWelfordCount=y+m;const d=u-this._taskDurationsWelfordMean;this._taskDurationsWelfordMean+=d*m/this._taskDurationsWelfordCount;const g=u-this._taskDurationsWelfordMean;this._taskDurationsWelfordM2+=d*g,u<this._taskDurationsMin&&(this._taskDurationsMin=u),u>this._taskDurationsMax&&(this._taskDurationsMax=u)}}catch(f){this._debugLog&&this._debugLog(f,"worker.onmessage: latency tracking inner")}}catch(c){this._debugLog&&this._debugLog(c,"worker.onmessage: latency tracking outer")}if(!this._queuePaused&&this.queue.length>0&&r.tasks<this._maxTasksPerWorker){const c=this.queue.shift();try{const u=w();c.transfer?t.postMessage(c.message,c.transfer):t.postMessage(c.message),r._startTimes.push(u),r.tasks++,this._activeTasks++}catch(u){this._debugLog&&this._debugLog(u,"dispatch queued message to worker failed"),this._logger.error(u,"Failed to dispatch queued message to worker")}this._queueHighCrossed&&this.queue.length<=this._queueHighThreshold&&(this._queueHighCrossed=!1)}if(this._onmessage)try{this._onmessage(s)}catch(c){this._logger.error(c,"Pool onmessage handler error")}try{this._bus.emit("message",s)}catch(c){this._logger.error(c,"pool listener error")}this._updateIdleState()};const i=s=>{let l=s&&s.data!==void 0?s.data:s,c=l;if(l&&(l instanceof ArrayBuffer||ArrayBuffer.isView(l)))try{c=j(l)}catch(f){try{o(f)}catch{}c=l}const u={data:c,originalEvent:s};if(typeof t.onmessage=="function")try{t.onmessage(u)}catch(f){this._logger.error(f,"worker wrapper onmessage error")}},a=s=>{if(typeof t.onerror=="function")try{t.onerror(s)}catch(l){this._logger.error(l,"worker wrapper onerror error")}try{this._bus.emit("error",s)}catch(l){this._logger.error(l,"pool error listener error")}},o=s=>{if(typeof t.onmessageerror=="function")try{t.onmessageerror(s)}catch(l){this._logger.error(l,"worker wrapper onmessageerror error")}try{this._bus.emit("messageerror",s)}catch(l){this._logger.error(l,"pool messageerror listener error")}};if(typeof n.addEventListener=="function"){try{n.addEventListener("message",i)}catch(s){this._debugLog&&this._debugLog(s,"attach addEventListener message")}try{n.addEventListener("error",a)}catch(s){this._debugLog&&this._debugLog(s,"attach addEventListener error")}try{n.addEventListener("messageerror",o)}catch(s){this._debugLog&&this._debugLog(s,"attach addEventListener messageerror")}}else if(typeof n.on=="function"){try{n.on("message",i)}catch(s){this._debugLog&&this._debugLog(s,"attach underlying.on message")}try{n.on("error",a)}catch(s){this._debugLog&&this._debugLog(s,"attach underlying.on error")}try{n.on("messageerror",o)}catch(s){this._debugLog&&this._debugLog(s,"attach underlying.on messageerror")}}else{try{n.onmessage=i}catch(s){this._debugLog&&this._debugLog(s,"assign underlying.onmessage")}try{n.onerror=a}catch(s){this._debugLog&&this._debugLog(s,"assign underlying.onerror")}try{n.onmessageerror=o}catch(s){this._debugLog&&this._debugLog(s,"assign underlying.onmessageerror")}}return r}_findLeastLoadedWorker(){if(!this.workers.length)return null;let e=null,n=1/0,t=Number.POSITIVE_INFINITY;for(let r=0;r<this.workers.length;r++){const i=this.workers[r],a=i.latencyEwma!=null?i.latencyEwma:Number.POSITIVE_INFINITY;(i.tasks<n||i.tasks===n&&a<t)&&(e=i,n=i.tasks,t=a)}return e}_handleUnderlyingMessage(e,n){const t=this._underlyingToWorkerObj.get(e);if(!t)return;const r=t.worker;let i=n&&n.data!==void 0?n.data:n,a=i;if(i&&(i instanceof ArrayBuffer||ArrayBuffer.isView(i)))try{a=j(i)}catch(s){try{this._handleUnderlyingMessageError(e,s)}catch{}a=i}const o={data:a,originalEvent:n};if(typeof r.onmessage=="function")try{r.onmessage(o)}catch(s){this._logger.error(s,"worker wrapper onmessage error")}}_handleUnderlyingError(e,n){const t=this._underlyingToWorkerObj.get(e);if(!t)return;const r=t.worker;if(typeof r.onerror=="function")try{r.onerror(n)}catch(i){this._logger.error(i,"worker wrapper onerror error")}try{this._bus.emit("error",n)}catch(i){this._logger.error(i,"pool error listener error")}}_handleUnderlyingMessageError(e,n){const t=this._underlyingToWorkerObj.get(e);if(!t)return;const r=t.worker;if(typeof r.onmessageerror=="function")try{r.onmessageerror(n)}catch(i){this._logger.error(i,"worker wrapper onmessageerror error")}try{this._bus.emit("messageerror",n)}catch(i){this._logger.error(i,"pool messageerror listener error")}}postMessage(e,n,t){t=t||void 0;const r=t&&t.workerId!=null?t.workerId:null,i=r!=null?this.workers.find(u=>u.id===r):this._findLeastLoadedWorker(),a=!!(t&&(t.awaitResponse||t.correlationId!=null));let o,s;if(a){if(o=t.correlationId!=null?String(t.correlationId):this._generateCorrelationId(),!(e!==null&&typeof e=="object"&&!ArrayBuffer.isView(e)&&!(e instanceof ArrayBuffer)))throw new Error("postMessage awaitResponse requires a plain-object message");e=Object.assign({},e,{correlationId:o}),s=new Promise((f,p)=>{const m={resolve:f,reject:p,timer:null},y=o!=null?String(o):o;t&&t.timeout&&(m.timer=setTimeout(()=>{try{this._cleanupPendingResponse(y,{rejectWith:new Error("postMessage response timeout")})}catch{try{p(new Error("postMessage response timeout"))}catch{}}},t.timeout)),this._pendingResponses.set(y,m)}),o=o!=null?String(o):o}if(i&&i.tasks<this._maxTasksPerWorker)try{const u=w(),f=this._prepareForTransfer(e,n,t);return f.transfer&&f.transfer.length?i.worker.postMessage(f.message,f.transfer):i.worker.postMessage(f.message),i._startTimes&&typeof i._startTimes.push=="function"&&i._startTimes.push(u),i.tasks++,this._activeTasks++,i.lastActive=u,this._updateIdleState(),a?s:!0}catch(u){if(a&&o){try{this._cleanupPendingResponse(o,{rejectWith:u})}catch{}return this._logger.error(u,"Failed to postMessage to worker"),s}return this._logger.error(u,"Failed to postMessage to worker"),!1}if(r!=null&&(!i||i.tasks>=this._maxTasksPerWorker)){if(a&&o){try{this._cleanupPendingResponse(o,{rejectWith:new Error("targeted worker unavailable")})}catch{}return s}return!1}if(r==null&&this.workers.length<this.maxSize){const u=this._addWorkerInstance();try{const f=w(),p=this._prepareForTransfer(e,n,t);return p.transfer&&p.transfer.length?u.worker.postMessage(p.message,p.transfer):u.worker.postMessage(p.message),u._startTimes&&typeof u._startTimes.push=="function"&&u._startTimes.push(f),u.tasks++,this._activeTasks++,u.lastActive=f,this._updateIdleState(),a?s:!0}catch(f){if(a&&o){try{this._cleanupPendingResponse(o,{rejectWith:f})}catch{}return this._logger.error(f,"Failed to postMessage to new worker"),s}return this._logger.error(f,"Failed to postMessage to new worker"),!1}}if(this.taskQueueEnabled){const u=this._prepareForTransfer(e,n,t),f=this._queuePolicy;if(f==="reject")return a&&o?(this._cleanupPendingResponse(o,{rejectWith:new Error("postMessage rejected by queue policy")}),s):!1;if(f==="drop-newest"&&this.queue.length>0)return a&&o?(this._cleanupPendingResponse(o,{rejectWith:new Error("postMessage rejected by queue policy")}),s):!1;if(f==="drop-oldest"&&this.queue.length>0){const m=this.queue.shift();m&&m.correlationId!=null&&this._cleanupPendingResponse(m.correlationId,{rejectWith:new Error("postMessage queued task dropped by policy")})}const p={message:u.message,transfer:u.transfer};a&&o&&(p.correlationId=o),this.queue.push(p);try{if(Number.isFinite(this._queueHighThreshold)&&this.queue.length>this._queueHighThreshold&&!this._queueHighCrossed){this._queueHighCrossed=!0;try{this._bus.emit("pool:queue:high",{length:this.queue.length,threshold:this._queueHighThreshold})}catch(m){this._logger.error(m,"pool queue high listener error")}}}catch{}return this._updateIdleState(),a?s:!0}if(!this.workers.length)return a?s:!1;const l=this._nextIndex%this.workers.length;this._nextIndex=(this._nextIndex+1)%this.workers.length;const c=this.workers[l];try{const u=w(),f=this._prepareForTransfer(e,n);return f.transfer&&f.transfer.length?c.worker.postMessage(f.message,f.transfer):c.worker.postMessage(f.message),c._startTimes&&typeof c._startTimes.push=="function"&&c._startTimes.push(u),c.tasks++,this._activeTasks++,c.lastActive=u,this._updateIdleState(),a?s:!0}catch(u){if(a&&o){try{this._cleanupPendingResponse(o,{rejectWith:u})}catch{}return this._logger.error(u,"Failed to postMessage to fallback worker"),s}return this._logger.error(u,"Failed to postMessage to fallback worker"),!1}}_generateCorrelationId(){try{if(typeof globalThis<"u"&&globalThis.crypto&&typeof globalThis.crypto.randomUUID=="function")return globalThis.crypto.randomUUID()}catch{}try{if(typeof globalThis<"u"&&globalThis.crypto&&typeof globalThis.crypto.getRandomValues=="function"){const n=new Uint8Array(16);return globalThis.crypto.getRandomValues(n),Array.from(n).map(t=>t.toString(16).padStart(2,"0")).join("")}}catch{}const e=Math.floor(Math.random()*4294967295).toString(16);return`cid-${Date.now().toString(36)}-${e}`}_cleanupPendingResponse(e,n={}){const t=e!=null?String(e):e,r=this._pendingResponses.get(t);if(!r)return!1;try{if(r.timer)try{clearTimeout(r.timer)}catch{}}catch{}try{Object.prototype.hasOwnProperty.call(n,"resolveWith")?r.resolve(n.resolveWith):Object.prototype.hasOwnProperty.call(n,"rejectWith")&&r.reject(n.rejectWith)}catch{}finally{try{this._pendingResponses.delete(t)}catch{}}return!0}broadcast(e,n){const t=w();let r=null;const i=e!==null&&typeof e=="object"&&!ArrayBuffer.isView(e)&&!(e instanceof ArrayBuffer);for(const a of this.workers)try{let o=e,s=n;if(!s&&i)try{r==null&&(r=this._encodeForTransfer(e));const l=r.slice();o=l,s=[l.buffer]}catch{o=e,s=void 0}s&&s.length?a.worker.postMessage(o,s):a.worker.postMessage(o),a._startTimes&&typeof a._startTimes.push=="function"&&a._startTimes.push(t),a.tasks++,this._activeTasks++,a.lastActive=t}catch(o){this._logger.error(o,"broadcast error")}this._updateIdleState()}stopThePress(e,n,t){const r=t&&typeof t.recreateWorkers<"u"?!!t.recreateWorkers:!0,i=t&&typeof t=="object"?Object.assign({},t):void 0;i&&delete i.recreateWorkers;try{this.queue&&typeof this.queue.clear=="function"&&this.queue.clear()}catch(s){this._logger.error(s,"stopThePress: failed to clear queue")}try{for(const[s]of this._pendingResponses)try{this._cleanupPendingResponse(s,{rejectWith:new Error("stopThePress: cancelled pending response")})}catch{}}catch(s){this._logger.error(s,"stopThePress: failed to cancel pending responses")}const a=this.workers.length,o=this.workers.map(s=>s&&s.id).filter(s=>s!=null);try{for(let s=this.workers.length-1;s>=0;s--){const l=this.workers[s];this._terminatedWorkerTaskCountsTotal+=l.completedTasks||0,this._terminatedWorkerTaskCountsCount+=1;try{l.worker.terminate()}catch{}this._deleteWorkerUnderlyingMapping(l)}this.workers.length=0,this._activeTasks=0}catch(s){this._logger.error(s,"stopThePress: failed while terminating workers")}if(r||this._clearLifecycleIntervals(),r){const s=Math.max(this.minSize,Math.min(a,this.maxSize));for(let l=0;l<s;l++)this._addWorkerInstance();try{this._ensureReaper()}catch{}}try{o&&o.length&&this._bus.emit("pool:scale",{action:"remove",terminated:o,count:a})}catch(s){this._logger.error(s,"pool scale stopThePress listener error")}return this._updateIdleState(),this.postMessage(e,n,i)}postMessageBatch(e,n){if(!Array.isArray(e))throw new Error("postMessageBatch expects an array of {message, transfer?}");const t=!!(n&&(n.awaitResponse||n.correlationId!=null)),r=n&&typeof n.correlationIdFactory=="function"?n.correlationIdFactory:null;if(t){if(n&&n.correlationId!=null&&e.length>1&&!r)throw new Error("postMessageBatch cannot use a fixed correlationId for multiple items; provide options.correlationIdFactory or omit correlationId");const u=new Array(e.length);for(let f=0;f<e.length;f++){const p=e[f]||{},m=Object.assign({},n);r&&(m.correlationId=String(r(f,p))),u[f]=this.postMessage(p.message,p.transfer,m)}return u}const i=new Array(e.length),a=[],o=n&&n.workerId!=null?n.workerId:null,s=this.prepareBuffers(e,{clone:!0,zeroCopy:n&&!!n.zeroCopy});let l=null;if(o!=null){if(l=this.workers.find(u=>u.id===o),!l)return e.map(()=>!1)}else l=this._findLeastLoadedWorker();let c=!1;for(let u=0;u<e.length;u++){const f=e[u]||{},p=s[u]||{message:f.message,transfer:f.transfer};let m=!1;l&&l.tasks>=this._maxTasksPerWorker&&(l=null);let y=l;if(!y&&o==null&&(y=this._findLeastLoadedWorker()),y&&y.tasks<this._maxTasksPerWorker)try{const d=w();p.transfer&&p.transfer.length?y.worker.postMessage(p.message,p.transfer):y.worker.postMessage(p.message),y._startTimes&&typeof y._startTimes.push=="function"&&y._startTimes.push(d),y.tasks++,this._activeTasks++,y.lastActive=d,c=!0,i[u]=!0,m=!0,l=y.tasks<this._maxTasksPerWorker?y:null}catch{i[u]=!1,m=!0}if(!m&&o==null&&this.workers.length<this.maxSize)try{const d=this._addWorkerInstance(),g=w();p.transfer&&p.transfer.length?d.worker.postMessage(p.message,p.transfer):d.worker.postMessage(p.message),d._startTimes&&typeof d._startTimes.push=="function"&&d._startTimes.push(g),d.tasks++,this._activeTasks++,d.lastActive=g,c=!0,i[u]=!0,m=!0,l=d.tasks<this._maxTasksPerWorker?d:null}catch{i[u]=!1,m=!0}if(!m){if(o!=null){i[u]=!1;continue}if(this.taskQueueEnabled){const d=this._queuePolicy;d==="reject"||d==="drop-newest"&&this.queue.length>0?i[u]=!1:(d==="drop-oldest"&&this.queue.length>0&&this.queue.shift(),a.push({message:p.message,transfer:p.transfer}),i[u]=!0)}else if(!this.workers.length)i[u]=!1;else{const d=this._nextIndex%this.workers.length;this._nextIndex=(this._nextIndex+1)%this.workers.length;const g=this.workers[d];try{const b=w();p.transfer&&p.transfer.length?g.worker.postMessage(p.message,p.transfer):g.worker.postMessage(p.message),g._startTimes&&typeof g._startTimes.push=="function"&&g._startTimes.push(b),g.tasks++,this._activeTasks++,g.lastActive=b,c=!0,i[u]=!0}catch(b){i[u]=!1,this._logger.error(b,"Failed to postMessage to fallback worker")}}}}if(a.length)try{this.queue.pushMany(a),c=!0}catch(u){this._logger.error(u,"postMessageBatch: failed to enqueue prepared items")}return c&&this._updateIdleState(),i}stopThePressBatch(e,n){const t=n&&typeof n.recreateWorkers<"u"?!!n.recreateWorkers:!0,r=n&&typeof n=="object"?Object.assign({},n):void 0;r&&delete r.recreateWorkers;try{this.queue&&typeof this.queue.clear=="function"&&this.queue.clear()}catch(a){this._logger.error(a,"stopThePressBatch: failed to clear queue")}try{for(const[a]of this._pendingResponses)try{this._cleanupPendingResponse(a,{rejectWith:new Error("stopThePressBatch: cancelled pending response")})}catch{}}catch(a){this._logger.error(a,"stopThePressBatch: failed to cancel pending responses")}const i=this.workers.length;try{for(let a=this.workers.length-1;a>=0;a--){const o=this.workers[a];this._terminatedWorkerTaskCountsTotal+=o.completedTasks||0,this._terminatedWorkerTaskCountsCount+=1;try{o.worker.terminate()}catch{}this._deleteWorkerUnderlyingMapping(o)}this.workers.length=0,this._activeTasks=0}catch(a){this._logger.error(a,"stopThePressBatch: failed while terminating workers")}if(t||this._clearLifecycleIntervals(),t){const a=Math.max(this.minSize,Math.min(i,this.maxSize));for(let o=0;o<a;o++)this._addWorkerInstance();try{this._ensureReaper()}catch{}}this._updateIdleState();try{return this.postMessageBatch(e,r)}catch(a){try{this._logger.error(a,"stopThePressBatch: postMessageBatch failed")}catch{}try{return new Array(e?e.length:0).fill(!1)}catch{return[]}}}addWorker(){return this._addWorkerInstance()}removeWorker(){const e=this.workers.pop();if(e){this._decrementActiveTasks(e.tasks||0);try{e.worker.terminate()}catch{}this._deleteWorkerUnderlyingMapping(e),this._terminatedWorkerTaskCountsTotal+=e.completedTasks||0,this._terminatedWorkerTaskCountsCount+=1}}_reapIdleWorkers(){if(this.idleTimeout<=0)return;const e=w();for(let n=this.workers.length-1;n>=0;n--){const t=this.workers[n];if(this.workers.length<=this.minSize)break;if(t.tasks===0&&e-(t.lastActive||0)>this.idleTimeout){try{t.worker.terminate()}catch{}try{const i=t.worker&&t.worker._underlying;i&&this._underlyingToWorkerObj&&this._underlyingToWorkerObj.delete(i)}catch{}const r=this.workers.length-1;n===r?this.workers.pop():this.workers[n]=this.workers.pop()}}this._updateIdleState()}_autoScaleTick(){try{if(!this._autoScale||!this._autoScale.enabled)return;const e=w(),n=this._autoScale;this._lastAutoScaleAt&&n.backoffResetMs&&e-this._lastAutoScaleAt>n.backoffResetMs&&(this._autoScaleBackoffMultiplier=1);const t=Math.floor((n.cooldownMs||0)*(this._autoScaleBackoffMultiplier||1));if(this._lastAutoScaleAt&&e-this._lastAutoScaleAt<t)return;const r=n.targetMs,i=n.hysteresis||.2,a=this._ewmaLatency,o=this.workers.length,s=r*(1+i),l=a!=null?a>s:!1,c=this.queue.length>Math.ceil(o*(1+i));if(l||c){if(o<this.maxSize)try{const p=Math.min(this.maxSize-o,n.stepUp||1);for(let m=0;m<p;m++)this._addWorkerInstance();this._lastAutoScaleAt=e,this._autoScaleBackoffMultiplier=Math.min(n.backoffMaxMultiplier||8,Math.max(1,(this._autoScaleBackoffMultiplier||1)*(n.backoffFactor||1)))}catch(p){this._debugLog&&this._debugLog(p,"autoScale: addWorker failed")}return}const u=r*Math.max(0,1-i);if((a!=null?a<u:!1)&&this.queue.length===0&&o>this.minSize)try{const p=Math.min(o-this.minSize,n.stepDown||1);for(let m=0;m<p;m++){const y=this.workers.pop();if(y){try{y.worker.terminate()}catch(d){this._debugLog&&this._debugLog(d,"autoScale: terminate worker")}this._deleteWorkerUnderlyingMapping(y)}}this._lastAutoScaleAt=e,this._autoScaleBackoffMultiplier=Math.min(n.backoffMaxMultiplier||8,Math.max(1,(this._autoScaleBackoffMultiplier||1)*(n.backoffFactor||1)))}catch(p){this._debugLog&&this._debugLog(p,"autoScale: remove worker failed")}}catch(e){this._debugLog&&this._debugLog(e,"autoScaleTick outer")}}_emitIdle(){const e={data:{type:"pool:idle",stats:this.getStats()}};if(this._isIdle=!0,this._onmessage)try{this._onmessage(e)}catch(n){this._logger.error(n,"Pool onmessage handler error")}if(this._onidle)try{this._onidle(e)}catch(n){this._logger.error(n,"Pool onidle handler error")}try{this._bus.emit("message",e)}catch(n){this._logger.error(n,"pool listener error")}try{this._bus.emit("idle",e)}catch(n){this._logger.error(n,"pool idle listener error")}}_updateIdleState(){const e=this.queue.length===0,t=this._activeTasks===0&&e;t&&!this._isIdle?this._emitIdle():!t&&this._isIdle&&(this._isIdle=!1)}terminate(){try{this.shutdown()}catch{}}[Symbol.dispose](){this.terminate()}async[Symbol.asyncDispose](){try{await this.drain()}catch{}this.terminate()}getStats(){const e=this.workers.map(b=>({id:b.id,tasks:b.tasks,lastActive:b.lastActive})),n=w(),t=this._createdAt!=null?Math.max(0,n-this._createdAt):0,r=this._totalWorkersCreated||this.workers.length,i=this._totalTasksCompleted||0,a=this._terminatedWorkerTaskCountsCount||0,o=this._terminatedWorkerTaskCountsTotal||0;let s=0;for(const b of this.workers)s+=b.completedTasks||0;const l=this.workers.length||0,c=a+l,u=c>0?(o+s)/c:0;let f=0,p=0,m=0,y=0,d=0;const g=this._taskDurationsWelfordCount||0;if(g>0){f=this._taskDurationsMin===Number.POSITIVE_INFINITY?0:this._taskDurationsMin,p=this._taskDurationsMax===Number.NEGATIVE_INFINITY?0:this._taskDurationsMax,m=this._taskDurationsWelfordMean;const b=g>1?this._taskDurationsWelfordM2/g:0;y=Math.sqrt(b),d=0}return{status:e,performance:{poolLiveDuration:t,totalWorkersCreated:r,totalTasksPerformed:i,averageTasksPerWorkerUntilTermination:u,timePerTask:{max:p,min:f,average:m,stddev:y},percentSlowTasks:d},queueLength:this.queue.length,activeTasks:this._activeTasks,workerCount:this.workers.length,minSize:this.minSize,maxSize:this.maxSize,isIdle:this._activeTasks===0&&this.queue.length===0}}drain(){const e=this.queue.length===0;return this._activeTasks===0&&e?Promise.resolve(this.getStats()):new Promise(r=>{const i=()=>{try{this.removeEventListener("idle",i)}catch{}r(this.getStats())};this.addEventListener("idle",i)})}addEventListener(e,n){if(typeof n=="function"&&(this._bus.on(e,n),e==="idle")){const t=this.queue.length===0;if(this._activeTasks===0&&t){const i={data:{type:"pool:idle",stats:this.getStats()}};try{n(i)}catch(a){this._logger.error(a,"pool idle listener error")}}}}removeEventListener(e,n){!n||typeof n!="function"||this._bus.off(e,n)}get onresize(){return this._onresize}set onresize(e){this._onresize=e}get onmessage(){return this._onmessage}set onmessage(e){this._onmessage=e}get onerror(){return this._onerror}set onerror(e){this._onerror=e}get onidle(){return this._onidle}set onidle(e){if(this._onidle=e,typeof e=="function"){const n=this.queue.length===0;if(this._activeTasks===0&&n){const r={data:{type:"pool:idle",stats:this.getStats()}};try{e(r)}catch(i){this._logger.error(i,"Pool onidle handler error")}}}}pauseQueue(){this._queuePaused=!0}resumeQueue(){this._queuePaused&&(this._queuePaused=!1,this._dispatchQueuedTasks())}pause(){return this.pauseQueue()}resume(){return this.resumeQueue()}get queuePaused(){return this._queuePaused}_dispatchQueuedTasks(){if(this._queuePaused||!this.taskQueueEnabled||this.queue.length===0)return;const e=w();let n=!1;for(const t of this.workers)for(;this.queue.length>0&&t.tasks<this._maxTasksPerWorker;){const r=this.queue.shift();try{r.transfer&&r.transfer.length?t.worker.postMessage(r.message,r.transfer):t.worker.postMessage(r.message),t._startTimes&&typeof t._startTimes.push=="function"&&t._startTimes.push(e),t.tasks++,this._activeTasks++,t.lastActive=e,n=!0}catch(i){this._debugLog&&this._debugLog(i,"dispatch queued message to worker failed"),this._logger.error(i,"Failed to dispatch queued message to worker");break}}this._queueHighCrossed&&this.queue.length<=this._queueHighThreshold&&(this._queueHighCrossed=!1),n&&this._updateIdleState()}}class D{constructor(e,n={}){if(typeof e!="function")throw new TypeError("PowerScheduler requires a flush function");this._flushFn=e,this._scheduling=n.scheduling==="macrotask"?"macrotask":"microtask",this._scheduled=!1,this._timer=null}get scheduled(){return this._scheduled}schedule(){if(!this._scheduled){if(this._scheduled=!0,this._scheduling==="macrotask"){this._timer=setTimeout(()=>this._run(),0);return}queueMicrotask(()=>this._run())}}flush(){this._scheduled&&(this._timer&&(clearTimeout(this._timer),this._timer=null),this._run())}cancel(){this._scheduled&&(this._scheduled=!1,this._timer&&(clearTimeout(this._timer),this._timer=null))}_run(){if(this._scheduled){this._scheduled=!1,this._timer=null;try{this._flushFn()}catch(e){console.error(e)}}}}const N=new WeakMap;let ne=0;const te=h=>(N.has(h)||N.set(h,String(ne++)),N.get(h));class re{constructor({map:e,source:n,sourceLayer:t,fid:r="id",tileSize:i=512,tolerance:a=1e-6,cacheSize:o=5e3,units:s="meters",postDelay:l=0,debugLevel:c=null,debuglevel:u=null,tilePoolSize:f=6,gatherPoolSize:p=4,gatherTimeout:m=6e4,mapFallbackCooldown:y=150,tileTimeout:d=null,tileMaxRetries:g=1,tileWorkerSource:b=null,gatherWorkerSource:C=null}){this.map=e,this.source=n,this.sourceLayer=t,this.fid=r,this.tileSize=i,this.tolerance=a,this.units=s,this.gatherPoolSize=Math.max(1,Number.isFinite(Number(p))?Math.floor(Number(p)):1),this.postDelay=Number.isFinite(Number(l))?Math.max(0,Number(l)):0;const A=c??u;this.debugLevel=Number.isFinite(Number(A))?Math.max(0,Math.min(3,Math.floor(Number(A)))):0,this.gatherTimeout=Number.isFinite(Number(m))?Math.max(0,Number(m)):3e4,this.tileTimeout=Number.isFinite(Number(d))?Math.max(0,Number(d)):this.gatherTimeout,this.tileMaxRetries=Number.isFinite(Number(g))?Math.max(0,Math.floor(Number(g))):1,this.mapFallbackCooldown=Number.isFinite(Number(y))?Math.max(0,Number(y)):150,this._lastMapFallbackAt=0,this._lastMapFallbackUnique=null,this._sourceLoaded=!1,this._pendingTiles=new Set,this._tilePendingMeta=new Map,this._tileCorrelationSeq=0,this._tileTimeoutHandle=null,this._tileQueue=new L(32),this._tileDrainScheduled=!1,this._tileDrainTimeout=null,this._tileScheduler=new D(()=>{this._tileDrainScheduled=!1,this._drainTileQueue()}),this._gatherRound=0,this._diffScheduler=new D(()=>this._runDiffFlush()),this._gatherScheduled=!1,this._diffScheduled=!1,this._diffFlushInProgress=!1,this._diffFlushQueued=!1,this._diffFlushQueuedGatherRound=0,this._currentFlushGatherRound=0,this._diffFlushQueuedTimestamp=0,this._currentFlushTimestamp=0,this._diffRetryHandle=null,this._diffRetryCount=0,this._lastGatherRound=0,this._lastGatherTimestamp=0,this._pendingGatherRounds=new Map,this._diffAdd=new Map,this._diffRemove=new Set,this._bus=new q,this._disposed=!1,this._changedGroups=new Set,this._tileGroups=new Map,this._groupToTiles=new Map,this.piecesCache=new E({maxEntries:o,maxWeight:o*5e3,weightFn:_=>_.size||0,onEvict:_=>{this._tileFingerprints?.delete(_),this._removeTileGroups(_)}}),this._tileFingerprints=new E({maxEntries:o,maxWeight:o,weightFn:()=>1,onEvict:_=>{this.piecesCache?.delete(_)}}),this._computeTileFingerprintMemo=new T(_=>this._computeTileFingerprintBody(_),{cacheOptions:{maxEntries:o},keyResolver:_=>Array.isArray(_)?_.map(W=>te(W)).join("|"):String(_)}),this.labelsCache=new E({maxEntries:o,maxWeight:o*5e3,weightFn:_=>Array.isArray(_)?_.length:0});const z=b,O=C;this.tilePool=new U(z,{size:f,minSize:1,maxSize:f,taskQueue:!0,lazy:!1,debugLevel:this.debugLevel}),this.gatherPool=new U(O,{size:p,minSize:1,maxSize:p,taskQueue:!0,lazy:!1,debugLevel:this.debugLevel}),this.tilePool.addEventListener("message",_=>this._onTileMessage(_)),this.tilePool.addEventListener("error",()=>this._expirePendingTiles(!0)),this.tilePool.addEventListener("messageerror",()=>this._expirePendingTiles(!0)),this.tilePool.addEventListener("idle",()=>{this._sourceLoaded&&this._changedGroups.size>0&&this._scheduleGather()}),this.gatherPool.addEventListener("message",_=>this._onGatherMessage(_)),this.gatherPool.addEventListener("idle",()=>this._scheduleDiffFlush()),this._bus.on("label",_=>this._collectLabelDiff(_)),this._bus.on("commit",_=>this._scheduleDiffFlush(_))}handleSourceData(e){if(this._disposed||!e||e.sourceId!==this.source.id)return;if(typeof e.isSourceLoaded=="boolean"){const g=this._sourceLoaded;this._sourceLoaded=e.isSourceLoaded,!g&&this._sourceLoaded&&this._changedGroups.size>0&&this._scheduleGather()}const n=e.tile?.tileID?.canonical;if(!n)return;const t=`${n.z}|${n.x}|${n.y}`;let r=[];const i=this.source.type==="vector"?{sourceLayer:this.sourceLayer}:{},a=typeof e.tile?.querySourceFeatures=="function"?g=>{const b=[];return e.tile.querySourceFeatures(b,g),b}:null,o=typeof this.map.querySourceFeatures=="function"?g=>this.map.querySourceFeatures(this.source.id,g):null,s=!a&&!!o;let l=t;if(s){l=`map|z${n.z}`;const g=Date.now();if(this._lastMapFallbackUnique===l&&g-this._lastMapFallbackAt<this.mapFallbackCooldown)return;this._lastMapFallbackUnique=l,this._lastMapFallbackAt=g}const c=a?a(i):o?o(i):null;if(!Array.isArray(c)||(r=c,!r.length))return;const u=this._computeTileFingerprint(r),f=this._tileFingerprints.get(l),p=f!=null&&f===u,m=this.piecesCache.has(l);if(this._pendingTiles.has(l)&&p||m&&p)return;m&&this.piecesCache.delete(l),this._tileFingerprints.set(l,u);const y=Math.max(this.tolerance,Math.pow(10,-.301*n.z+2.56)/this.tileSize),d={collection:{type:"FeatureCollection",features:r.map((g,b)=>({id:g.properties?.[this.fid]??g.id,geometry:g.geometry,properties:{...g.properties,_index:`${l}|${b}`,_tile:l,_group:g.properties?.[this.fid]}}))},tolerance:y,unique:l,tileSize:this.tileSize,debugLevel:this.debugLevel};d.correlationId=`${l}:${++this._tileCorrelationSeq}`,this._pendingTiles.add(l),this._tilePendingMeta.set(l,{correlationId:d.correlationId,payload:d,retries:0,deadline:Date.now()+Math.max(1,this.tileTimeout)}),this._tileQueue.push(d),this._schedulePendingTileTimeoutCheck(),this._scheduleTileDrain()}_computeTileFingerprint(e){return this._computeTileFingerprintMemo(e)}_computeTileFingerprintBody(e){let n=2166136261;const t=r=>{const i=String(r);for(let a=0;a<i.length;a+=1)n^=i.charCodeAt(a),n=n*16777619>>>0};for(const r of e){const i=r.properties?.[this.fid]??r.id,a=r.geometry;let o=0;if(a&&a.coordinates&&(o=this._countCoordinates(a.coordinates)),t(i),t(a?.type||"none"),t(o),r.properties&&typeof r.properties=="object"){const s=Object.keys(r.properties).sort();for(const l of s)t(l),t(":"),t(this._serializePropertyValue(r.properties[l])),t("|")}}return n.toString(16).padStart(8,"0")}_countCoordinates(e){if(!Array.isArray(e))return 0;let n=0;if(Array.isArray(e[0])&&Array.isArray(e[0][0])){for(const t of e)n+=this._countCoordinates(t);return n}return e.length}_removeTileGroups(e){const n=this._tileGroups.get(e);if(n){for(const t of n){const r=this._groupToTiles.get(t);r&&(r.delete(e),r.size===0&&this._groupToTiles.delete(t))}this._tileGroups.delete(e)}}_serializePropertyValue(e){return e==null||typeof e=="string"||typeof e=="number"||typeof e=="boolean"?String(e):Array.isArray(e)?`array:${e.length}`:typeof e=="object"?`object:${Object.keys(e).length}`:String(e)}setGeoJsonSource(e){this.gjSource=e}dispose(){this._disposed=!0,this._tileScheduler.cancel(),this._diffScheduler.cancel(),this._tileDrainTimeout&&(clearTimeout(this._tileDrainTimeout),this._tileDrainTimeout=null),this._tileTimeoutHandle&&(clearTimeout(this._tileTimeoutHandle),this._tileTimeoutHandle=null),this._diffRetryHandle&&(clearTimeout(this._diffRetryHandle),this._diffRetryHandle=null);for(const e of this._pendingGatherRounds.values())clearTimeout(e);this._pendingGatherRounds.clear(),this._tileDrainScheduled=!1,this._gatherScheduled=!1,this._diffScheduled=!1;try{this.tilePool.shutdown()}catch{}try{this.gatherPool.shutdown()}catch{}this.piecesCache.clear(),this.labelsCache.clear(),this._tileFingerprints.clear(),this._pendingTiles.clear(),this._tilePendingMeta.clear(),this._tileQueue.clear(),this._changedGroups.clear(),this._tileGroups.clear(),this._groupToTiles.clear(),this._bus.clear(),this.gjSource=null}_schedulePendingTileTimeoutCheck(){if(this._tileTimeoutHandle||this._tilePendingMeta.size===0||this.tileTimeout<=0)return;const e=Math.max(25,Math.min(1e3,Math.floor(this.tileTimeout/2)||250));this._tileTimeoutHandle=setTimeout(()=>{this._tileTimeoutHandle=null,this._expirePendingTiles(!1)},e)}_expirePendingTiles(e=!1){if(this._disposed||this._tilePendingMeta.size===0)return;const n=Date.now();let t=!1;for(const[r,i]of this._tilePendingMeta.entries())!e&&i.deadline>n||(i.retries<this.tileMaxRetries?(i.retries+=1,i.deadline=n+Math.max(1,this.tileTimeout),i.correlationId=`${r}:${++this._tileCorrelationSeq}`,i.payload={...i.payload,correlationId:i.correlationId},this._tileQueue.push(i.payload),t=!0):(this._tilePendingMeta.delete(r),this._pendingTiles.delete(r)));t&&this._scheduleTileDrain(),this._tilePendingMeta.size>0&&this._schedulePendingTileTimeoutCheck()}_scheduleTileDrain(){if(!(this._tileDrainScheduled||this._tileDrainTimeout)){if(this._tileDrainScheduled=!0,this.postDelay>0){this._tileDrainTimeout=setTimeout(()=>{this._tileDrainTimeout=null,this._tileDrainScheduled=!1,this._drainTileQueue()},this.postDelay);return}this._tileScheduler.schedule()}}_drainTileQueue(){const e=[];let n;for(;(n=this._tileQueue.shift())!==void 0;)e.push({message:n});e.length>0&&this.tilePool.postMessageBatch(e,{zeroCopy:!0})}_onTileMessage(e){const n=e.data;if(!n||n.type!=="simplified")return;const{unique:t,correlationId:r,...i}=n,a=this._tilePendingMeta.get(t);if(a){if(r!=null&&a.correlationId!==r)return;this._tilePendingMeta.delete(t),this._pendingTiles.delete(t)}else if(this._pendingTiles.has(t))this._pendingTiles.delete(t);else if(r!=null)return;this._tilePendingMeta.size===0&&this._tileTimeoutHandle&&(clearTimeout(this._tileTimeoutHandle),this._tileTimeoutHandle=null),this._removeTileGroups(t),this.piecesCache.set(t,i),this._tileGroups.set(t,new Set);for(const o of Object.keys(i)){const s=this._groupToTiles.get(o)||new Set;s.add(t),this._groupToTiles.set(o,s),this._tileGroups.get(t).add(o),this._changedGroups.add(o)}this._sourceLoaded&&this._scheduleGather()}_scheduleGather(){this._gatherScheduled||(this._gatherScheduled=!0,queueMicrotask(()=>{this._gatherScheduled=!1,this._dispatchGather()}))}_dispatchGather(){if(this._disposed)return;let e;if(this._changedGroups.size>0){const t=new Set;for(const r of this._changedGroups){const i=this._groupToTiles.get(r);if(i)for(const a of i)t.add(a)}if(t.size===0){this._changedGroups.clear();return}e={};for(const r of t){const i=this.piecesCache.get(r);i&&(e[r]=i)}this._changedGroups.clear()}else{const t=Array.from(this.piecesCache.entries("LRU"));if(!t.length)return;e=Object.fromEntries(t)}if(!e||!Object.keys(e).length)return;const n=++this._gatherRound;this._scheduleGatherTimeout(n),this.gatherPool.postMessage({pieces:e,tolerance:this.tolerance,unit:this.units,tileSize:this.tileSize,gatherPoolSize:this.gatherPoolSize,debugLevel:this.debugLevel,gatherRound:n},void 0,{zeroCopy:!0})}_scheduleGatherTimeout(e){if(this.gatherTimeout<=0)return;const n=setTimeout(()=>{this._pendingGatherRounds.has(e)&&(this._pendingGatherRounds.delete(e),this._scheduleDiffFlush({gatherRound:e,timestamp:Date.now()}))},this.gatherTimeout);this._pendingGatherRounds.set(e,n)}_clearGatherTimeout(e){const n=this._pendingGatherRounds.get(e);n&&(clearTimeout(n),this._pendingGatherRounds.delete(e))}_onGatherMessage(e){const n=e.data;if(n){if(n.type==="commit"){const t=Number.isFinite(Number(n.gatherRound))?Number(n.gatherRound):0,r=Number.isFinite(Number(n.timestamp))?Number(n.timestamp):Date.now();t&&this._clearGatherTimeout(t),this._bus.emit("commit",{gatherRound:t,timestamp:r});return}n.id!=null&&this._bus.emit("label",n)}}_keepNonContained=e=>{const n=e.map(r=>r.split("-")),t=(r,i)=>{let a=0;for(const o of i)if(o===r[a]&&(a+=1),a===r.length)return!0;return r.length===0};return n.filter((r,i)=>!n.some((a,o)=>o!==i&&a.length>=r.length&&t(r,a))).map(r=>r.join("-"))};_filterRedundantDiffAdds(e){if(!Array.isArray(e)||e.length===0)return[];const n=new Map;let t=!1,r=!1;for(const o of e){const s=o?.properties?._index;if(s){if(!r&&String(s).includes("-")&&(r=!0),!t){const l=o?.properties?._members;Array.isArray(l)&&l.length>1&&(t=!0)}n.set(s,o)}}if(n.size===0)return[];if(n.size===1)return[n.values().next().value];if(t){const o=new Set;for(const s of n.values()){const l=s?.properties?._index,c=s?.properties?._members;if(!(!l||!Array.isArray(c)||c.length<=1))for(const u of c)u!==l&&n.has(u)&&o.add(u)}if(o.size>0){const s=[];for(const l of n.values())o.has(l?.properties?._index)||s.push(l);return s}}if(!r)return[...n.values()];const i=new Set(this._keepNonContained([...n.keys()].sort())),a=[];for(const[o,s]of n.entries())i.has(o)&&a.push(s);return a}_collectLabelDiff(e){const n=Number.isFinite(Number(e.gatherRound))?Number(e.gatherRound):0,t=Number.isFinite(Number(e.timestamp))?Number(e.timestamp):0;if(n&&n<this._lastGatherRound)return;n&&n>this._lastGatherRound&&(this._lastGatherRound=n,this._lastGatherTimestamp=0),t&&t>this._lastGatherTimestamp&&(this._lastGatherTimestamp=t);const r=e.id;let i=Array.isArray(e.features)?e.features.slice():[];if(i.length>1&&i.sort((o,s)=>{const l=o?.properties?._index,c=s?.properties?._index;return l===c?0:l==null?-1:c==null?1:String(l).localeCompare(String(c))}),this.labelsCache.hasEqual(r,i))return;const a=this.labelsCache.get(r);a&&a.forEach(o=>{o?.properties?._index&&this._diffRemove.add(o.properties._index)}),i.forEach(o=>{o?.properties?._index&&this._diffAdd.set(o.properties._index,o)}),this.labelsCache.set(r,i),this._scheduleDiffFlush({gatherRound:n,timestamp:t})}_scheduleDiffFlush({gatherRound:e=0,timestamp:n=0}={}){if(e&&e>this._lastGatherRound&&(this._lastGatherRound=e,this._lastGatherTimestamp=0),n&&n>this._lastGatherTimestamp&&(this._lastGatherTimestamp=n),!(!this._diffFlushInProgress&&!this._diffScheduled&&this._diffAdd.size===0&&this._diffRemove.size===0)&&!(!this._diffFlushInProgress&&!this._diffScheduled&&e&&e<this._lastGatherRound)){if(this._diffFlushInProgress){if(e&&e<=this._currentFlushGatherRound||n&&n<=this._currentFlushTimestamp)return;this._diffFlushQueued=!0,e&&e>this._diffFlushQueuedGatherRound&&(this._diffFlushQueuedGatherRound=e),n&&n>this._diffFlushQueuedTimestamp&&(this._diffFlushQueuedTimestamp=n);return}if(this._diffScheduled){e&&e>this._diffFlushQueuedGatherRound&&(this._diffFlushQueuedGatherRound=e),n&&n>this._diffFlushQueuedTimestamp&&(this._diffFlushQueuedTimestamp=n);return}this._diffScheduled=!0,this._diffScheduler.schedule()}}async _runDiffFlush(){if(!this._diffScheduled||(this._diffScheduled=!1,this._diffFlushInProgress))return;this._diffFlushInProgress=!0;const e=Math.max(this._lastGatherRound,this._diffFlushQueuedGatherRound),n=Math.max(this._lastGatherTimestamp,this._diffFlushQueuedTimestamp,Date.now());this._currentFlushGatherRound=e,this._currentFlushTimestamp=n;const t=this._diffFlushQueuedGatherRound,r=this._diffFlushQueuedTimestamp;this._diffFlushQueuedGatherRound=0,this._diffFlushQueuedTimestamp=0;let i=!0;try{i=await this._flushDiffs()}finally{this._diffFlushInProgress=!1,this._currentFlushGatherRound=0,this._currentFlushTimestamp=0,i||this._scheduleDiffRetry(),this._diffFlushQueued&&(this._diffFlushQueued=!1,this._scheduleDiffFlush({gatherRound:t,timestamp:r}))}}_scheduleDiffRetry(){if(this._disposed||this._diffRetryHandle||this._diffAdd.size===0&&this._diffRemove.size===0)return;const e=Math.min(1e3,Math.max(25,25*Math.pow(2,this._diffRetryCount)));this._diffRetryCount+=1,this._diffRetryHandle=setTimeout(()=>{this._diffRetryHandle=null,this._scheduleDiffFlush({gatherRound:this._lastGatherRound,timestamp:Date.now()})},e)}async _flushDiffs(){if(this._disposed||!this.gjSource||this._diffAdd.size===0&&this._diffRemove.size===0)return!0;const e=this._diffAdd.size>0?[...this._diffAdd.entries()]:[];let n=[];if(e.length===1)n=[e[0][1]];else if(e.length>1){const i=e.map(([,a])=>a);n=this._filterRedundantDiffAdds(i)}const t=this._diffRemove.size>0?[...this._diffRemove]:[];let r=null;if(t.length>0)if(n.length===0)r=t;else{const i=new Set;for(const a of n){const o=a?.properties?._index;o&&i.add(o)}r=[];for(const a of t)i.has(a)||r.push(a)}try{await this.gjSource.updateData({add:n,remove:r||[]})}catch{return!1}for(const[i,a]of e)i&&this._diffAdd.get(i)===a&&this._diffAdd.delete(i);for(const i of t)this._diffRemove.delete(i);return this._diffRetryCount=0,this._diffRetryHandle&&(clearTimeout(this._diffRetryHandle),this._diffRetryHandle=null),!0}}const G=`let v, M;
function oe() {
...
`,V=typeof self<"u"&&self.Blob&&new Blob(["URL.revokeObjectURL(import.meta.url);",G],{type:"text/javascript;charset=utf-8"});function ie(h){let e;try{if(e=V&&(self.URL||self.webkitURL).createObjectURL(V),!e)throw"";const n=new Worker(e,{type:"module",name:h?.name});return n.addEventListener("error",()=>{(self.URL||self.webkitURL).revokeObjectURL(e)}),n}catch{return new Worker("data:text/javascript;charset=utf-8,"+encodeURIComponent(G),{type:"module",name:h?.name})}}const $=`let ue, he;
function qt() {
