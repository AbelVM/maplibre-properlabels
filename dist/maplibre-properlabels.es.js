const rn = 23283064365386963e-26, Sn = 12, sn = typeof TextDecoder > "u" ? null : new TextDecoder("utf-8"), J = 0, H = 1, q = 2, z = 5;
class Pn {
  /**
  ...
  readFields(n, e, s = this.length) {
    for (; this.pos < s; ) {
      const i = this.readVarint(), l = i >> 3, a = this.pos;
      this.type = i & 7, n(l, e, this), this.pos === a && this.skip(i);
    }
    return e;
  }
  /**
  ...
  readVarint(n) {
    const e = this.buf;
    let s, i;
    return i = e[this.pos++], s = i & 127, i < 128 || (i = e[this.pos++], s |= (i & 127) << 7, i < 128) || (i = e[this.pos++], s |= (i & 127) << 14, i < 128) || (i = e[this.pos++], s |= (i & 127) << 21, i < 128) ? s : (i = e[this.pos], s |= (i & 15) << 28, Fn(s, n, this));
  }
  readVarint64() {
  ...
  readString() {
    ...
    return this.pos = n, n - e >= Sn && sn ? sn.decode(this.buf.subarray(e, n)) : Cn(this.buf, e, n);
  }
  readBytes() {
  ...
  /** @param {number} val */
  writeFixed64(n) {
    this.realloc(8), this.dataView.setInt32(this.pos, n & -1, !0), this.dataView.setInt32(this.pos + 4, Math.floor(n * rn), !0), this.pos += 8;
  }
  /** @param {number} val */
  writeSFixed64(n) {
    this.realloc(8), this.dataView.setInt32(this.pos, n & -1, !0), this.dataView.setInt32(this.pos + 4, Math.floor(n * rn), !0), this.pos += 8;
  ...
  writeString(n) {
    ...
    const e = this.pos;
    this.pos = In(this.buf, n, this.pos);
    const s = this.pos - e;
    s >= 128 && on(e, s, this), this.pos = e - 1, this.writeVarint(s), this.pos += s;
  }
  /** @param {number} val */
  writeFloat(n) {
  ...
  writeRawMessage(n, e) {
    ...
    n(e, this);
    const i = this.pos - s;
    i >= 128 && on(s, i, this), this.pos = s - 1, this.writeVarint(i), this.pos += i;
  }
  /**
   * @template T
   * @param {number} tag
  ...
  writePackedVarint(n, e) {
    e.length && this.writeMessage(n, An, e);
  }
  /**
   * @param {number} tag
   * @param {number[]} arr
  ...
  writePackedSVarint(n, e) {
    e.length && this.writeMessage(n, Ln, e);
  }
  /**
   * @param {number} tag
   * @param {boolean[]} arr
  ...
  writePackedDouble(n, e) {
    e.length && this.writeMessage(n, Tn, e);
  }
  /**
   * @param {number} tag
   * @param {number[]} arr
...
function Fn(r, n, e) {
  const s = e.buf;
  let i, l;
  if (l = s[e.pos++], i = (l & 112) >> 4, l < 128 || (l = s[e.pos++], i |= (l & 127) << 3, l < 128) || (l = s[e.pos++], i |= (l & 127) << 10, l < 128) || (l = s[e.pos++], i |= (l & 127) << 17, l < 128) || (l = s[e.pos++], i |= (l & 127) << 24, l < 128) || (l = s[e.pos++], i |= (l & 1) << 31, l < 128)) return D(r, i, n);
...
function Cn(r, n, e) {
  ...
  for (; i < e; ) {
    const l = r[i];
    let a = null, p = l > 239 ? 4 : l > 223 ? 3 : l > 191 ? 2 : 1;
    if (i + p > e) break;
    let c, g, y;
    p === 1 ? l < 128 && (a = l) : p === 2 ? (c = r[i + 1], (c & 192) === 128 && (a = (l & 31) << 6 | c & 63, a <= 127 && (a = null))) : p === 3 ? (c = r[i + 1], g = r[i + 2], (c & 192) === 128 && (g & 192) === 128 && (a = (l & 15) << 12 | (c & 63) << 6 | g & 63, (a <= 2047 || a >= 55296 && a <= 57343) && (a = null))) : p === 4 && (c = r[i + 1], g = r[i + 2], y = r[i + 3], (c & 192) === 128 && (g & 192) === 128 && (y & 192) === 128 && (a = (l & 15) << 18 | (c & 63) << 12 | (g & 63) << 6 | y & 63, (a <= 65535 || a >= 1114112) && (a = null))), a === null ? (a = 65533, p = 1) : a > 65535 && (a -= 65536, s += String.fromCharCode(a >>> 10 & 1023 | 55296), a = 56320 | a & 1023), s += String.fromCharCode(a), i += p;
  }
  return s;
}
function In(r, n, e) {
...
R.convert = function(r) {
  ...
  if (Array.isArray(r))
    return new R(+r[0], +r[1]);
  if (r.x !== void 0 && r.y !== void 0)
    return new R(+r.x, +r.y);
...
class xn {
  ...
  constructor(n, e, s, i, l) {
    this.properties = {}, this.extent = s, this.type = 0, this.id = void 0, this._pbf = n, this._geometry = -1, this._keys = i, this._values = l, n.readFields(Dn, this, e);
  }
  loadGeometry() {
    const n = this._pbf;
    n.pos = this._geometry;
    const e = n.readVarint() + n.pos, s = [];
    let i, l = 1, a = 0, p = 0, c = 0;
    for (; n.pos < e; ) {
      if (a <= 0) {
      ...
      if (a--, l === 1 || l === 2)
        p += n.readSVarint(), c += n.readSVarint(), l === 1 && (i && s.push(i), i = []), i && i.push(new R(p, c));
      else if (l === 7)
        i && i.push(i[0].clone());
      else
  ...
  bbox() {
    ...
    let s = 1, i = 0, l = 0, a = 0, p = 1 / 0, c = -1 / 0, g = 1 / 0, y = -1 / 0;
    for (; n.pos < e; ) {
      if (i <= 0) {
      ...
      if (i--, s === 1 || s === 2)
        l += n.readSVarint(), a += n.readSVarint(), l < p && (p = l), l > c && (c = l), a < g && (g = a), a > y && (y = a);
      else if (s !== 7)
   ...
   */
  toGeoJSON(n, e, s) {
    const i = this.extent * Math.pow(2, s), l = this.extent * n, a = this.extent * e, p = this.loadGeometry();
    function c(u) {
      return [
        (u.x + l) * 360 / i - 180,
        360 / Math.PI * Math.atan(Math.exp((1 - (u.y + a) * 2 / i) * Math.PI)) - 90
      ];
    }
    function g(u) {
      return u.map(c);
...
xn.types = ["Unknown", "Point", "LineString", "Polygon"];
function Dn(r, n, e) {
  r === 1 ? n.id = e.readVarint() : r === 2 ? Gn(e, n) : r === 3 ? n.type = /** @type {0 | 1 | 2 | 3} */
...
function Un(r) {
  ...
  for (let l = 0; l < n; l++) {
    const a = qn(r[l]);
    a !== 0 && (i === void 0 && (i = a < 0), i === a < 0 ? (s && e.push(s), s = [r[l]]) : s && s.push(r[l]));
  }
  return s && e.push(s), e;
}
function qn(r) {
  let n = 0;
  for (let e = 0, s = r.length, i = s - 1, l, a; e < s; i = e++)
    l = r[e], a = r[i], n += (a.x - l.x) * (l.y + a.y);
  return n;
}
class jn {
  /**
   ...
   */
  constructor(n, e) {
    this.version = 1, this.name = "", this.extent = 4096, this.length = 0, this._pbf = n, this._keys = [], this._values = [], this._features = [], n.readFields(Hn, this, e), this.length = this._features.length;
  }
  ...
  feature(n) {
    ...
    return new xn(this._pbf, e, this.extent, this._keys, this._values);
  }
}
function Hn(r, n, e) {
  r === 15 ? n.version = e.readVarint() : r === 1 ? n.name = e.readString() : r === 5 ? n.extent = e.readVarint() : r === 2 ? n._features.push(e.pos) : r === 3 ? n._keys.push(e.readString()) : r === 4 && n._values.push(zn(e));
}
function zn(r) {
  let n = null;
  const e = r.readVarint() + r.pos;
...
class Kn {
   ...
   */
  constructor(n, e) {
    this.layers = n.readFields($n, {}, e);
  }
}
function $n(r, n, e) {
  if (r === 3) {
    const s = new jn(e, e.readVarint() + e.pos);
    s.length && (n[s.name] = s);
  }
}
function Wn(r) {
...
var an;
function Jn() {
  return an || (an = 1, K.read = function(r, n, e, s, i) {
    var l, a, p = i * 8 - s - 1, c = (1 << p) - 1, g = c >> 1, y = -7, x = e ? i - 1 : 0, u = e ? -1 : 1, f = r[n + x];
    for (x += u, l = f & (1 << -y) - 1, f >>= -y, y += p; y > 0; l = l * 256 + r[n + x], x += u, y -= 8)
      ;
    for (a = l & (1 << -y) - 1, l >>= -y, y += s; y > 0; a = a * 256 + r[n + x], x += u, y -= 8)
      ;
    if (l === 0)
      l = 1 - g;
    else {
      ...
      a = a + Math.pow(2, s), l = l - g;
    }
    return (f ? -1 : 1) * a * Math.pow(2, l - s);
  }, K.write = function(r, n, e, s, i, l) {
    var a, p, c, g = l * 8 - i - 1, y = (1 << g) - 1, x = y >> 1, u = i === 23 ? Math.pow(2, -24) - Math.pow(2, -77) : 0, f = s ? 0 : l - 1, d = s ? 1 : -1, w = n < 0 || n === 0 && 1 / n < 0 ? 1 : 0;
    for (n = Math.abs(n), isNaN(n) || n === 1 / 0 ? (p = isNaN(n) ? 1 : 0, a = y) : (a = Math.floor(Math.log(n) / Math.LN2), n * (c = Math.pow(2, -a)) < 1 && (a--, c *= 2), a + x >= 1 ? n += u / c : n += u * Math.pow(2, 1 - x), n * c >= 2 && (a++, c /= 2), a + x >= y ? (p = 0, a = y) : a + x >= 1 ? (p = (n * c - 1) * Math.pow(2, i), a = a + x) : (p = n * Math.pow(2, x - 1) * Math.pow(2, i), a = 0)); i >= 8; r[e + f] = p & 255, f += d, p /= 256, i -= 8)
      ;
    for (a = a << i | p, g += i; g > 0; r[e + f] = a & 255, f += d, a /= 256, g -= 8)
      ;
    r[e + f - d] |= w * 128;
  }), K;
}
...
function Xn() {
  ...
  n.prototype = {
    ...
    readFields: function(t, o, h) {
      for (h = h || this.length; this.pos < h; ) {
        var v = this.readVarint(), S = v >> 3, A = this.pos;
        this.type = v & 7, t(S, o, this), this.pos === A && this.skip(v);
      }
      return o;
    },
    ...
    readFixed64: function() {
      var t = F(this.buf, this.pos) + F(this.buf, this.pos + 4) * e;
      return this.pos += 8, t;
    },
    readSFixed64: function() {
      var t = F(this.buf, this.pos) + k(this.buf, this.pos + 4) * e;
      return this.pos += 8, t;
    },
    readFloat: function() {
    ...
    readVarint: function(t) {
      var o = this.buf, h, v;
      return v = o[this.pos++], h = v & 127, v < 128 || (v = o[this.pos++], h |= (v & 127) << 7, v < 128) || (v = o[this.pos++], h |= (v & 127) << 14, v < 128) || (v = o[this.pos++], h |= (v & 127) << 21, v < 128) ? h : (v = o[this.pos], h |= (v & 15) << 28, a(h, t, this));
    },
    ...
    readString: function() {
      ...
      return this.pos = t, t - o >= i && l ? U(this.buf, o, t) : O(this.buf, o, t);
    },
    readBytes: function() {
    ...
    writeFixed32: function(t) {
      this.realloc(4), E(this.buf, t, this.pos), this.pos += 4;
    },
    writeSFixed32: function(t) {
      this.realloc(4), E(this.buf, t, this.pos), this.pos += 4;
    },
    writeFixed64: function(t) {
      this.realloc(8), E(this.buf, t & -1, this.pos), E(this.buf, Math.floor(t * s), this.pos + 4), this.pos += 8;
    },
    ...
    writeString: function(t) {
      ...
      var o = this.pos;
      this.pos = C(this.buf, t, this.pos);
      var h = this.pos - o;
      h >= 128 && u(o, h, this), this.pos = o - 1, this.writeVarint(h), this.pos += h;
    },
    ...
    writePackedSVarint: function(t, o) {
      o.length && this.writeMessage(t, d, o);
    },
    ...
    writePackedFixed64: function(t, o) {
      o.length && this.writeMessage(t, _, o);
    },
    writePackedSFixed64: function(t, o) {
      o.length && this.writeMessage(t, L, o);
    },
  ...
  function a(t, o, h) {
    var v = h.buf, S, A;
    if (A = v[h.pos++], S = (A & 112) >> 4, A < 128 || (A = v[h.pos++], S |= (A & 127) << 3, A < 128) || (A = v[h.pos++], S |= (A & 127) << 10, A < 128) || (A = v[h.pos++], S |= (A & 127) << 17, A < 128) || (A = v[h.pos++], S |= (A & 127) << 24, A < 128) || (A = v[h.pos++], S |= (A & 1) << 31, A < 128)) return c(t, S, o);
    throw new Error("Expected varint not more than 10 bytes");
  }
  function p(t) {
  ...
  function u(t, o, h) {
    ...
    h.realloc(v);
    for (var S = h.pos - 1; S >= t; S--) h.buf[S + v] = h.buf[S];
  }
  function f(t, o) {
  ...
  function O(t, o, h) {
    for (var v = "", S = o; S < h; ) {
      var A = t[S], T = null, V = A > 239 ? 4 : A > 223 ? 3 : A > 191 ? 2 : 1;
      if (S + V > h) break;
      var B, I, W;
      V === 1 ? A < 128 && (T = A) : V === 2 ? (B = t[S + 1], (B & 192) === 128 && (T = (A & 31) << 6 | B & 63, T <= 127 && (T = null))) : V === 3 ? (B = t[S + 1], I = t[S + 2], (B & 192) === 128 && (I & 192) === 128 && (T = (A & 15) << 12 | (B & 63) << 6 | I & 63, (T <= 2047 || T >= 55296 && T <= 57343) && (T = null))) : V === 4 && (B = t[S + 1], I = t[S + 2], W = t[S + 3], (B & 192) === 128 && (I & 192) === 128 && (W & 192) === 128 && (T = (A & 15) << 18 | (B & 63) << 12 | (I & 63) << 6 | W & 63, (T <= 65535 || T >= 1114112) && (T = null))), T === null ? (T = 65533, V = 1) : T > 65535 && (T -= 65536, v += String.fromCharCode(T >>> 10 & 1023 | 55296), T = 56320 | T & 1023), v += String.fromCharCode(T), S += V;
    }
    return v;
  }
  function U(t, o, h) {
  ...
  function C(t, o, h) {
    for (var v = 0, S, A; v < o.length; v++) {
      if (S = o.charCodeAt(v), S > 55295 && S < 57344)
        if (A)
          if (S < 56320) {
            t[h++] = 239, t[h++] = 191, t[h++] = 189, A = S;
            continue;
          } else
            S = A - 55296 << 10 | S - 56320 | 65536, A = null;
        else {
          S > 56319 || v + 1 === o.length ? (t[h++] = 239, t[h++] = 191, t[h++] = 189) : A = S;
          continue;
        }
      else A && (t[h++] = 239, t[h++] = 191, t[h++] = 189, A = null);
      S < 128 ? t[h++] = S : (S < 2048 ? t[h++] = S >> 6 | 192 : (S < 65536 ? t[h++] = S >> 12 | 224 : (t[h++] = S >> 18 | 240, t[h++] = S >> 12 & 63 | 128), t[h++] = S >> 6 & 63 | 128), t[h++] = S & 63 | 128);
    }
    return h;
  }
  return X;
...
function wn() {
  ...
  return r.prototype = {
    ...
    _rotateAround: function(n, e) {
      var s = Math.cos(n), i = Math.sin(n), l = e.x + s * (this.x - e.x) - i * (this.y - e.y), a = e.y + i * (this.x - e.x) + s * (this.y - e.y);
...
function mn() {
  ...
  var r = wn();
  Z = n;
  function n(a, p, c, g, y) {
    this.properties = {}, this.extent = c, this.type = 0, this._pbf = a, this._geometry = -1, this._keys = g, this._values = y, a.readFields(e, this, p);
  }
  function e(a, p, c) {
    a == 1 ? p.id = c.readVarint() : a == 2 ? s(c, p) : a == 3 ? p.type = c.readVarint() : a == 4 && (p._geometry = c.pos);
  }
  function s(a, p) {
    for (var c = a.readVarint() + a.pos; a.pos < c; ) {
      var g = p._keys[a.readVarint()], y = p._values[a.readVarint()];
      p.properties[g] = y;
    }
  }
  n.types = ["Unknown", "Point", "LineString", "Polygon"], n.prototype.loadGeometry = function() {
    ...
    for (var p = a.readVarint() + a.pos, c = 1, g = 0, y = 0, x = 0, u = [], f; a.pos < p; ) {
      if (g <= 0) {
        var d = a.readVarint();
        c = d & 7, g = d >> 3;
      }
      if (g--, c === 1 || c === 2)
        y += a.readSVarint(), x += a.readSVarint(), c === 1 && (f && u.push(f), f = []), f.push(new r(y, x));
      else if (c === 7)
        f && f.push(f[0].clone());
      else
    ...
    var a = this._pbf;
    a.pos = this._geometry;
    for (var p = a.readVarint() + a.pos, c = 1, g = 0, y = 0, x = 0, u = 1 / 0, f = -1 / 0, d = 1 / 0, w = -1 / 0; a.pos < p; ) {
      if (g <= 0) {
        var M = a.readVarint();
        c = M & 7, g = M >> 3;
      }
      if (g--, c === 1 || c === 2)
        y += a.readSVarint(), x += a.readSVarint(), y < u && (u = y), y > f && (f = y), x < d && (d = x), x > w && (w = x);
      else if (c !== 7)
        throw new Error("unknown command " + c);
    }
    return [u, d, f, w];
  }, n.prototype.toGeoJSON = function(a, p, c) {
    var g = this.extent * Math.pow(2, c), y = this.extent * a, x = this.extent * p, u = this.loadGeometry(), f = n.types[this.type], d, w;
    function M(b) {
      for (var _ = 0; _ < b.length; _++) {
        var L = b[_], F = 180 - (L.y + x) * 360 / g;
        b[_] = [
          (L.x + y) * 360 / g - 180,
    ...
    switch (this.type) {
      ...
      case 2:
        ...
        break;
      case 3:
        for (u = i(u), d = 0; d < u.length; d++)
          for (w = 0; w < u[d].length; w++)
            M(u[d][w]);
        break;
    }
  ...
  function i(a) {
    var p = a.length;
    if (p <= 1) return [a];
    for (var c = [], g, y, x = 0; x < p; x++) {
      var u = l(a[x]);
      u !== 0 && (y === void 0 && (y = u < 0), y === u < 0 ? (g && c.push(g), g = [a[x]]) : g.push(a[x]));
    }
    return g && c.push(g), c;
  }
  function l(a) {
    for (var p = 0, c = 0, g = a.length, y = g - 1, x, u; c < g; y = c++)
      x = a[c], u = a[y], p += (u.x - x.x) * (x.y + u.y);
    return p;
  }
  return Z;
}
...
function bn() {
  ...
  function e(i, l, a) {
    i === 15 ? l.version = a.readVarint() : i === 1 ? l.name = a.readString() : i === 5 ? l.extent = a.readVarint() : i === 2 ? l._features.push(a.pos) : i === 3 ? l._keys.push(a.readString()) : i === 4 && l._values.push(s(a));
  }
  function s(i) {
    for (var l = null, a = i.readVarint() + i.pos; i.pos < a; ) {
      var p = i.readVarint() >> 3;
      l = p === 1 ? i.readString() : p === 2 ? i.readFloat() : p === 3 ? i.readDouble() : p === 4 ? i.readVarint64() : p === 5 ? i.readVarint() : p === 6 ? i.readSVarint() : p === 7 ? i.readBoolean() : null;
    }
    return l;
  }
...
function Yn() {
  ...
  function e(s, i, l) {
    if (s === 3) {
      var a = new r(l, l.readVarint() + l.pos);
      a.length && (i[a.name] = a);
    }
  }
  return nn;
...
var pn;
function Zn() {
  return pn || (pn = 1, j.VectorTile = Yn(), j.VectorTileFeature = mn(), j.VectorTileLayer = bn()), j;
}
var en, dn;
function Qn() {
  if (dn) return en;
  dn = 1;
  var r = wn(), n = Zn().VectorTileFeature;
  en = e;
  function e(i, l) {
  ...
  return s.prototype.loadGeometry = function() {
    ...
    for (var l = 0; l < i.length; l++) {
      for (var a = i[l], p = [], c = 0; c < a.length; c++)
        p.push(new r(a[c][0], a[c][1]));
      this.geometry.push(p);
    }
    return this.geometry;
  }, s.prototype.bbox = function() {
    this.geometry || this.loadGeometry();
    for (var i = this.geometry, l = 1 / 0, a = -1 / 0, p = 1 / 0, c = -1 / 0, g = 0; g < i.length; g++)
      for (var y = i[g], x = 0; x < y.length; x++) {
        var u = y[x];
        l = Math.min(l, u.x), a = Math.max(a, u.x), p = Math.min(p, u.y), c = Math.max(c, u.y);
      }
    return [l, p, a, c];
...
var gn;
function ne() {
  if (gn) return G.exports;
  gn = 1;
  var r = Xn(), n = Qn();
  G.exports = e, G.exports.fromVectorTileJs = e, G.exports.fromGeojsonVt = s, G.exports.GeoJSONWrapper = n;
  function e(u) {
    var f = new r();
    return i(u, f), f.finish();
  }
  function s(u, f) {
    f = f || {};
    var d = {};
    for (var w in u)
      d[w] = new n(u[w].features, f), d[w].name = w, d[w].version = f.version, d[w].extent = f.extent;
    return e({ layers: d });
  }
  function i(u, f) {
    for (var d in u.layers)
      f.writeMessage(3, l, u.layers[d]);
  }
  function l(u, f) {
    ...
    for (d = 0; d < u.length; d++)
      w.feature = u.feature(d), f.writeMessage(2, a, w);
    var M = w.keys;
    for (d = 0; d < M.length; d++)
      f.writeStringField(3, M[d]);
    var m = w.values;
    for (d = 0; d < m.length; d++)
      f.writeMessage(4, x, m[d]);
  }
  function a(u, f) {
    var d = u.feature;
    d.id !== void 0 && f.writeVarintField(1, d.id), f.writeMessage(2, p, u), f.writeVarintField(3, d.type), f.writeMessage(4, y, d);
  }
  function p(u, f) {
    var d = u.feature, w = u.keys, M = u.values, m = u.keycache, P = u.valuecache;
    for (var b in d.properties) {
      var _ = d.properties[b], L = m[b];
      if (_ !== null) {
        typeof L > "u" && (w.push(b), L = w.length - 1, m[b] = L), f.writeVarint(L);
        var F = typeof _;
        F !== "string" && F !== "boolean" && F !== "number" && (_ = JSON.stringify(_));
        var E = F + ":" + _, k = P[E];
        typeof k > "u" && (M.push(_), k = M.length - 1, P[E] = k), f.writeVarint(k);
      }
    }
  }
  function c(u, f) {
  ...
  function y(u, f) {
    for (var d = u.loadGeometry(), w = u.type, M = 0, m = 0, P = d.length, b = 0; b < P; b++) {
      var _ = d[b], L = 1;
      w === 1 && (L = _.length), f.writeVarint(c(1, L));
      for (var F = w === 3 ? _.length - 1 : _.length, E = 0; E < F; E++) {
        E === 1 && w !== 1 && f.writeVarint(c(2, F - 1));
        var k = _[E].x - M, O = _[E].y - m;
        f.writeVarint(g(k)), f.writeVarint(g(O)), M += k, m += O;
      }
      w === 3 && f.writeVarint(c(7, 1));
    }
  }
  function x(u, f) {
    var d = typeof u;
    d === "string" ? f.writeStringField(1, u) : d === "boolean" ? f.writeBooleanField(7, u) : d === "number" && (u % 1 !== 0 ? f.writeDoubleField(3, u) : u < 0 ? f.writeSVarintField(6, u) : f.writeVarintField(5, u));
  }
  return G.exports;
}
var ee = ne();
const te = /* @__PURE__ */ Wn(ee), vn = `var $t = /^-?(?:\\d+(?:\\.\\d*)?|\\.\\d+)(?:e[+-]?\\d+)?$/i, Ye = Math.ceil, re = Math.floor, Q = "[BigNumber Error] ", ct = Q + "Number primitive has more than 15 significant digits: ", se = 1e14, G = 14, Je = 9007199254740991, We = [1, 10, 100, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9, 1e10, 1e11, 1e12, 1e13], ye = 1e7, X = 1e9;
function kt(t) {
...
`, yn = typeof self < "u" && self.Blob && new Blob(["URL.revokeObjectURL(import.meta.url);", vn], { type: "text/javascript;charset=utf-8" });
function re(r) {
  let n;
  try {
    ...
    return new Worker(
      "data:text/javascript;charset=utf-8," + encodeURIComponent(vn),
      {
        type: "module",
        name: r?.name
      }
...
let N = !1;
function ie(r, n = {}) {
  const e = [], s = [], i = [], l = new TextEncoder(), a = [], p = /* @__PURE__ */ new Map();
  let c = 0, g = 0;
  const y = (w) => {
    if (Array.isArray(w)) {
      const M = Number(w[0]), m = Number(w[1]);
      s.push(Number.isFinite(M) ? M : 0, Number.isFinite(m) ? m : 0);
    } else if (w && (typeof w.x == "number" || typeof w.y == "number")) {
      const M = Number(w.x), m = Number(w.y);
      s.push(Number.isFinite(M) ? M : 0, Number.isFinite(m) ? m : 0);
    } else
      s.push(0, 0);
  };
  for (const w of r) {
    const M = w.id == null ? "" : String(w.id), m = w.geometry || {}, P = m.type || "Unknown", b = { id: M, type: P, coordsOffset: c, coordsLength: 0 };
    if (P === "Point") {
      const F = m.coordinates || [];
      y(F), b.coordsLength = 2;
    } else if (P === "LineString" || P === "MultiPoint") {
      const F = m.coordinates || [];
      for (const E of F) y(E);
      b.coordsLength = (F.length || 0) * 2;
    } else if (P === "Polygon") {
      const F = m.coordinates || [];
      b.ringLengths = [];
      for (const E of F) {
        b.ringLengths.push(E.length || 0);
        for (const k of E) y(k);
      }
      b.coordsLength = b.ringLengths.reduce((E, k) => E + k, 0) * 2;
    } else if (P === "MultiPolygon") {
      const F = m.coordinates || [];
      b.polygonRingCounts = [], b.ringLengths = [];
      for (const E of F) {
        b.polygonRingCounts.push(E.length || 0);
        for (const k of E) {
          b.ringLengths.push(k.length || 0);
          for (const O of k) y(O);
        }
      }
      b.coordsLength = b.ringLengths.reduce((E, k) => E + k, 0) * 2;
    } else
    ...
    for (const F of Object.keys(_)) {
      let E = p.get(F);
      E === void 0 && (E = a.length, a.push(F), p.set(F, E));
      const k = JSON.stringify(_[F]), O = l.encode(k);
      i.push(O), L.push([E, g, O.length]), g += O.length;
    }
    b.props = L, c += b.coordsLength, e.push(b);
  }
  let x;
  if (n.propsBuffer)
    n.propsBuffer instanceof Uint8Array ? x = n.propsBuffer.subarray(0, g) : x = new Uint8Array(n.propsBuffer, 0, g), x.byteLength < g && (x = new Uint8Array(g));
  else if (n.pool) {
    const w = n.pool.rent(g || 1);
    x = new Uint8Array(w, 0, g);
  } else
    x = new Uint8Array(g);
  let u = 0;
  for (const w of i)
    x.set(w, u), u += w.length;
  const f = s.length;
  let d;
  if (n.coordsBuffer)
    n.coordsBuffer instanceof ArrayBuffer ? d = new Float32Array(n.coordsBuffer, 0, f) : n.coordsBuffer instanceof Float32Array ? d = n.coordsBuffer.subarray(0, f) : d = new Float32Array(f), d.length < f && (d = new Float32Array(f));
  else if (n.pool) {
    const w = n.pool.rent(f * 4 || 4);
    d = new Float32Array(w, 0, f);
  } else
    d = new Float32Array(f);
  return d.length > 0 && d.set(s), { meta: e, keys: a, propsBuffer: x, coordsArray: d };
}
function tn(r, n, e, s) {
  const i = e instanceof Float32Array ? e : new Float32Array(e), l = n instanceof Uint8Array ? n : n ? new Uint8Array(n) : new Uint8Array(0), a = new TextDecoder(), p = [];
  for (let c = 0; c < (r.length || 0); c++) {
    const g = r[c] || {}, y = g.id, x = {};
    if (Array.isArray(g.props) && g.props.length && s && s.length)
      for (const m of g.props) {
        const [P, b, _] = m;
        try {
          const L = l.subarray(b, b + _);
          x[s[P]] = JSON.parse(a.decode(L));
        } catch {
        }
      }
    const u = g.type || "Unknown";
    let f = g.coordsOffset || 0;
    const d = f + (g.coordsLength || 0);
    let w = null;
    if (u === "Point") {
      const m = i[f], P = i[f + 1], b = Number.isFinite(m) ? Math.max(-180, Math.min(180, m)) : 0, _ = Number.isFinite(P) ? Math.max(-90, Math.min(90, P)) : 0;
      if ((!Number.isFinite(m) || !Number.isFinite(P)) && !N) {
        N = !0;
        try {
          console.warn("decodeFeaturesBinary: encountered non-finite coordinate, replacing with safe value", { index: c, id: y, rawX: m, rawY: P });
        } catch {
        }
      }
      w = { type: "Point", coordinates: [b, _] };
    } else if (u === "LineString" || u === "MultiPoint") {
      const m = [];
      for (; f < d; f += 2) {
        const P = i[f], b = i[f + 1], _ = Number.isFinite(P) ? Math.max(-180, Math.min(180, P)) : 0, L = Number.isFinite(b) ? Math.max(-90, Math.min(90, b)) : 0;
        if ((!Number.isFinite(P) || !Number.isFinite(b)) && !N) {
          N = !0;
          try {
            console.warn("decodeFeaturesBinary: encountered non-finite coordinate in linestring/multipoint, replacing with safe value", { index: c, id: y, rawX: P, rawY: b });
          } catch {
          }
        }
        m.push([_, L]);
      }
      w = { type: u, coordinates: m };
    } else if (u === "Polygon") {
      const m = [], P = g.ringLengths || [];
      for (const b of P) {
        const _ = [];
        for (let L = 0; L < b; L++) {
          const F = i[f], E = i[f + 1], k = Number.isFinite(F) ? Math.max(-180, Math.min(180, F)) : 0, O = Number.isFinite(E) ? Math.max(-90, Math.min(90, E)) : 0;
          if ((!Number.isFinite(F) || !Number.isFinite(E)) && !N) {
            N = !0;
            try {
              console.warn("decodeFeaturesBinary: encountered non-finite coordinate in polygon, replacing with safe value", { index: c, id: y, rawX: F, rawY: E });
            } catch {
            }
          }
          _.push([k, O]), f += 2;
      ...
      w = { type: "Polygon", coordinates: m };
    } else if (u === "MultiPolygon") {
      const m = [], P = g.polygonRingCounts || [], b = g.ringLengths || [];
      let _ = 0;
      for (const L of P) {
        const F = [];
        for (let E = 0; E < L; E++) {
          const k = b[_++] || 0, O = [];
          for (let U = 0; U < k; U++) {
            const C = i[f], t = i[f + 1], o = Number.isFinite(C) ? Math.max(-180, Math.min(180, C)) : 0, h = Number.isFinite(t) ? Math.max(-90, Math.min(90, t)) : 0;
            if ((!Number.isFinite(C) || !Number.isFinite(t)) && !N) {
              N = !0;
              try {
                console.warn("decodeFeaturesBinary: encountered non-finite coordinate in multipolygon, replacing with safe value", { index: c, id: y, rawX: C, rawY: t });
              } catch {
              }
            }
            O.push([o, h]), f += 2;
      ...
      w = { type: "MultiPolygon", coordinates: m };
    } else if (f < d) {
      const m = i[f], P = i[f + 1], b = Number.isFinite(m) ? Math.max(-180, Math.min(180, m)) : 0, _ = Number.isFinite(P) ? Math.max(-90, Math.min(90, P)) : 0;
      if ((!Number.isFinite(m) || !Number.isFinite(P)) && !N) {
        N = !0;
        try {
          console.warn("decodeFeaturesBinary: encountered non-finite coordinate in fallback path, replacing with safe value", { index: c, id: y, rawX: m, rawY: P });
        } catch {
        }
      }
      w = { type: "Point", coordinates: [b, _] };
    }
    w == null && (w = { type: "Point", coordinates: [0, 0] });
    const M = x && typeof x == "object" ? x : {};
    p.push({ type: "Feature", id: y, geometry: w, properties: M });
  }
  return p;
}
class se {
  constructor(n) {
    return this.map = n.map, this.source = n.source instanceof maplibregl.VectorTileSource ? n.source : this.map.getSource(n.source), this.sourceLayer = n.sourceLayer, this.fid = n.fid || "id", this.tiles = this.source.tiles.map((e) => e.split("{z}")[0]), this.tileSize = this.source.tileSize || 512, this.tolerance = n.tolerance || 1e-5, this.cacheSize = n.cacheSize || 1e4, this.minion = new re(), this._abPool = new $(), this.minion.onmessage = (e) => {
      const s = e && e.data;
      if (s)
        if (s.type === "geojson_bin" && s.coords)
          try {
            const i = s.coords instanceof Uint8Array ? s.coords.buffer : s.coords, l = s.propsBuf !== void 0 ? s.propsBuf : null, a = tn(s.meta || [], l, i, s.keys || []);
          ...
          try {
            ...
            if (s.add && s.add.coords)
              try {
                const u = s.add.propsBuf !== void 0 ? s.add.propsBuf : null, f = s.add.coords;
                a = tn(s.add.meta || [], u, f, s.add.keys || []);
                try {
                ...
                console.warn("Failed to decode add-list from worker", u);
                try {
                  this.minion.postMessage({ type: "request_full" });
                } catch {
                }
            ...
            if (s.update && s.update.coords)
              try {
                const u = s.update.propsBuf !== void 0 ? s.update.propsBuf : null, f = s.update.coords;
                p = tn(s.update.meta || [], u, f, s.update.keys || []);
                try {
                  u && this._abPool.release(u instanceof ArrayBuffer ? u : u.buffer);
                } catch {
                }
                ...
                console.warn("Failed to decode update-list from worker", u);
                try {
                  this.minion.postMessage({ type: "request_full" });
                } catch {
                }
            ...
            if (s.updateDiffs && Array.isArray(s.updateDiffs))
              ...
              try {
                const u = s.updateKeys || [], f = s.updatePropsBuf !== void 0 ? s.updatePropsBuf : null, d = f ? f instanceof Uint8Array ? f : new Uint8Array(f) : new Uint8Array(0), w = new TextDecoder();
                for (const M of s.updateDiffsMeta) {
                  const m = { id: M.id };
                  if (M.removeAllProperties && (m.removeAllProperties = !0), Array.isArray(M.removeProperties) && M.removeProperties.length && (m.removeProperties = M.removeProperties.map((P) => u[P])), Array.isArray(M.addOrUpdate) && M.addOrUpdate.length) {
                    const P = [];
                    for (const b of M.addOrUpdate) {
                      const [_, L, F] = b, E = u[_];
                      try {
                        const k = d.subarray(L, L + F), O = JSON.parse(w.decode(k));
                        P.push({ key: E, value: O });
                      } catch {
                      }
                    }
                    P.length && (m.addOrUpdateProperties = P);
                  }
                  c.push(m);
                }
                try {
                ...
                console.warn("Failed to decode compacted update diffs", u);
              }
            const g = new Map((p || []).map((u) => [String(u.id), u])), y = c.map((u) => {
              const f = { id: u.id }, d = g.get(String(u.id));
              return d && d.geometry && (f.newGeometry = d.geometry), u.removeAllProperties && (f.removeAllProperties = !0), u.removeProperties && (f.removeProperties = u.removeProperties), u.addOrUpdateProperties && (f.addOrUpdateProperties = u.addOrUpdateProperties), f;
            }).filter((u) => u != null), x = {};
            if (l ? x.removeAll = !0 : i.length && (x.remove = i), a.length && (x.add = a), y.length && (x.update = y), this.gjsource && typeof this.gjsource.updateData == "function")
              try {
          ...
          try {
            const i = s.payload instanceof Uint8Array ? s.payload.buffer : s.payload, l = new TextDecoder().decode(i), a = JSON.parse(l);
      ...
      if (e.sourceId === this.source.id && e.isSourceLoaded) {
        const s = this.map.querySourceFeatures(this.source.id, { sourceLayer: this.sourceLayer }), i = e.tile.tileID.canonical.z, l = this.tolerance * Math.pow(10, -0.301 * i + 5.19), a = {
          features: s.map((p) => ({ id: p.id, geometry: p.geometry, properties: p.properties })),
          tolerance: l
        };
        this._pendingPost = a, this._postTimer == null && (this._postTimer = setTimeout(() => {
          try {
            if (this._pendingPost)
              try {
                const { meta: p, keys: c, propsBuffer: g, coordsArray: y } = ie(this._pendingPost.features || [], { pool: this._abPool });
                this.minion.postMessage({ type: "features_bin", meta: p, keys: c, propsBuf: g.buffer, tolerance: this._pendingPost.tolerance, coords: y.buffer, cacheSize: this.cacheSize, promoteId: this.fid }, [g.buffer, y.buffer]);
              } catch {
                try {
                  const c = new TextEncoder(), g = Object.assign({}, this._pendingPost, { promoteId: this.fid }), y = JSON.stringify(g), x = c.encode(y);
                  this.minion.postMessage({ type: "features", payload: x.buffer }, [x.buffer]);
                } catch {
  ...
  _protocol = async (n) => {
    ...
    const a = i.length, [p, c, g] = i.slice(a - 4, a - 1).map((w) => w * 1), y = await l.arrayBuffer(), x = new Kn(new Pn(y)), u = {
      layers: Object.entries(x.layers).reduce((w, [M, m]) => ({
        ...w,
        [M]: {
          ...m,
          feature: (P) => {
            const b = m.feature(P), L = b.loadGeometry().flat(1 / 0).some(
              (F) => F.x >= m.extent - 1 || F.y >= m.extent - 1 || F.x <= 1 || F.y <= 1
            );
            return b.properties.clipped = L, b;
          }
        }
      }), {})
    };
    return { data: te(u).buffer };
  };
}
maplibregl.VectorTileSource.prototype.ProperLabels = function(r) {
  return this._proper || (this._proper = new se({
    map: this._map,
    source: this
  })), this._proper;
};
