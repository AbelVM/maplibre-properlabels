const hn = 23283064365386963e-26, Ln = 12, fn = typeof TextDecoder > "u" ? null : new TextDecoder("utf-8"), Q = 0, J = 1, K = 2, W = 5;
class Tn {
  /**
  ...
  readString() {
    ...
    return this.pos = n, n - e >= Ln && fn ? fn.decode(this.buf.subarray(e, n)) : zn(this.buf, e, n);
  }
  readBytes() {
...
function zn(r, n, e) {
  ...
  for (; i < e; ) {
    ...
    if (i + c > e) break;
    let p, m, g;
    c === 1 ? u < 128 && (l = u) : c === 2 ? (p = r[i + 1], (p & 192) === 128 && (l = (u & 31) << 6 | p & 63, l <= 127 && (l = null))) : c === 3 ? (p = r[i + 1], m = r[i + 2], (p & 192) === 128 && (m & 192) === 128 && (l = (u & 15) << 12 | (p & 63) << 6 | m & 63, (l <= 2047 || l >= 55296 && l <= 57343) && (l = null))) : c === 4 && (p = r[i + 1], m = r[i + 2], g = r[i + 3], (p & 192) === 128 && (m & 192) === 128 && (g & 192) === 128 && (l = (u & 15) << 18 | (p & 63) << 12 | (m & 63) << 6 | g & 63, (l <= 65535 || l >= 1114112) && (l = null))), l === null ? (l = 65533, c = 1) : l > 65535 && (l -= 65536, s += String.fromCharCode(l >>> 10 & 1023 | 55296), l = 56320 | l & 1023), s += String.fromCharCode(l), i += c;
  }
  return s;
}
function Kn(r, n, e) {
...
class Fn {
  ...
  bbox() {
    ...
    let s = 1, i = 0, u = 0, l = 0, c = 1 / 0, p = -1 / 0, m = 1 / 0, g = -1 / 0;
    for (; n.pos < e; ) {
      if (i <= 0) {
      ...
      if (i--, s === 1 || s === 2)
        u += n.readSVarint(), l += n.readSVarint(), u < c && (c = u), u > p && (p = u), l < m && (m = l), l > g && (g = l);
      else if (s !== 7)
...
Fn.types = ["Unknown", "Point", "LineString", "Polygon"];
function $n(r, n, e) {
...
class Yn {
  ...
  feature(n) {
    ...
    return new Fn(this._pbf, e, this.extent, this._keys, this._values);
  }
}
function Zn(r, n, e) {
...
function re() {
  return pn || (pn = 1, X.read = function(r, n, e, s, i) {
    var u, l, c = i * 8 - s - 1, p = (1 << c) - 1, m = p >> 1, g = -7, y = e ? i - 1 : 0, o = e ? -1 : 1, f = r[n + y];
    for (y += o, u = f & (1 << -g) - 1, f >>= -g, g += c; g > 0; u = u * 256 + r[n + y], y += o, g -= 8)
      ;
    for (l = u & (1 << -g) - 1, u >>= -g, g += s; g > 0; l = l * 256 + r[n + y], y += o, g -= 8)
      ;
    if (u === 0)
      u = 1 - m;
    else {
      ...
      l = l + Math.pow(2, s), u = u - m;
    }
    return (f ? -1 : 1) * l * Math.pow(2, u - s);
  }, X.write = function(r, n, e, s, i, u) {
    var l, c, p, m = u * 8 - i - 1, g = (1 << m) - 1, y = g >> 1, o = i === 23 ? Math.pow(2, -24) - Math.pow(2, -77) : 0, f = s ? 0 : u - 1, d = s ? 1 : -1, v = n < 0 || n === 0 && 1 / n < 0 ? 1 : 0;
    for (n = Math.abs(n), isNaN(n) || n === 1 / 0 ? (c = isNaN(n) ? 1 : 0, l = g) : (l = Math.floor(Math.log(n) / Math.LN2), n * (p = Math.pow(2, -l)) < 1 && (l--, p *= 2), l + y >= 1 ? n += o / p : n += o * Math.pow(2, 1 - y), n * p >= 2 && (l++, p /= 2), l + y >= g ? (c = 0, l = g) : l + y >= 1 ? (c = (n * p - 1) * Math.pow(2, i), l = l + y) : (c = n * Math.pow(2, y - 1) * Math.pow(2, i), l = 0)); i >= 8; r[e + f] = c & 255, f += d, c /= 256, i -= 8)
      ;
    for (l = l << i | c, m += i; m > 0; r[e + f] = l & 255, f += d, l /= 256, m -= 8)
      ;
    r[e + f - d] |= v * 128;
  }), X;
}
...
function ie() {
  ...
  n.prototype = {
    ...
    readString: function() {
      ...
      return this.pos = t, t - a >= i && u ? B(this.buf, a, t) : L(this.buf, a, t);
    },
    readBytes: function() {
    ...
    writePackedDouble: function(t, a) {
      a.length && this.writeMessage(t, x, a);
    },
    ...
    writePackedSFixed64: function(t, a) {
      a.length && this.writeMessage(t, k, a);
    },
  ...
  function L(t, a, h) {
    for (var P = "", b = a; b < h; ) {
      ...
      if (b + G > h) break;
      var C, U, Z;
      G === 1 ? E < 128 && (N = E) : G === 2 ? (C = t[b + 1], (C & 192) === 128 && (N = (E & 31) << 6 | C & 63, N <= 127 && (N = null))) : G === 3 ? (C = t[b + 1], U = t[b + 2], (C & 192) === 128 && (U & 192) === 128 && (N = (E & 15) << 12 | (C & 63) << 6 | U & 63, (N <= 2047 || N >= 55296 && N <= 57343) && (N = null))) : G === 4 && (C = t[b + 1], U = t[b + 2], Z = t[b + 3], (C & 192) === 128 && (U & 192) === 128 && (Z & 192) === 128 && (N = (E & 15) << 18 | (C & 63) << 12 | (U & 63) << 6 | Z & 63, (N <= 65535 || N >= 1114112) && (N = null))), N === null ? (N = 65533, G = 1) : N > 65535 && (N -= 65536, P += String.fromCharCode(N >>> 10 & 1023 | 55296), N = 56320 | N & 1023), P += String.fromCharCode(N), b += G;
    }
    return P;
  }
  function B(t, a, h) {
...
function Mn() {
  ...
  var r = En();
  tn = n;
  function n(l, c, p, m, g) {
    this.properties = {}, this.extent = p, this.type = 0, this._pbf = l, this._geometry = -1, this._keys = m, this._values = g, l.readFields(e, this, c);
  }
  function e(l, c, p) {
  ...
  function s(l, c) {
    for (var p = l.readVarint() + l.pos; l.pos < p; ) {
      var m = c._keys[l.readVarint()], g = c._values[l.readVarint()];
      c.properties[m] = g;
    }
  }
  n.types = ["Unknown", "Point", "LineString", "Polygon"], n.prototype.loadGeometry = function() {
    ...
    for (var c = l.readVarint() + l.pos, p = 1, m = 0, g = 0, y = 0, o = [], f; l.pos < c; ) {
      if (m <= 0) {
        var d = l.readVarint();
        p = d & 7, m = d >> 3;
      }
      if (m--, p === 1 || p === 2)
        g += l.readSVarint(), y += l.readSVarint(), p === 1 && (f && o.push(f), f = []), f.push(new r(g, y));
      else if (p === 7)
        f && f.push(f[0].clone());
      else
    ...
    var l = this._pbf;
    l.pos = this._geometry;
    for (var c = l.readVarint() + l.pos, p = 1, m = 0, g = 0, y = 0, o = 1 / 0, f = -1 / 0, d = 1 / 0, v = -1 / 0; l.pos < c; ) {
      if (m <= 0) {
        var x = l.readVarint();
        p = x & 7, m = x >> 3;
      }
      if (m--, p === 1 || p === 2)
        g += l.readSVarint(), y += l.readSVarint(), g < o && (o = g), g > f && (f = g), y < d && (d = y), y > v && (v = y);
      else if (p !== 7)
        throw new Error("unknown command " + p);
    }
    return [o, d, f, v];
  }, n.prototype.toGeoJSON = function(l, c, p) {
    var m = this.extent * Math.pow(2, p), g = this.extent * l, y = this.extent * c, o = this.loadGeometry(), f = n.types[this.type], d, v;
    function x(F) {
      for (var w = 0; w < F.length; w++) {
        var k = F[w], T = 180 - (k.y + y) * 360 / m;
        F[w] = [
          (k.x + g) * 360 / m - 180,
    ...
    return "id" in this && (_.id = this.id), _;
  };
  function i(l) {
    var c = l.length;
    if (c <= 1) return [l];
    for (var p = [], m, g, y = 0; y < c; y++) {
      var o = u(l[y]);
      o !== 0 && (g === void 0 && (g = o < 0), g === o < 0 ? (m && p.push(m), m = [l[y]]) : m.push(l[y]));
    }
    return m && p.push(m), p;
  }
  function u(l) {
    for (var c = 0, p = 0, m = l.length, g = m - 1, y, o; p < m; g = p++)
      y = l[p], o = l[g], c += (o.x - y.x) * (y.y + o.y);
    return c;
  }
  return tn;
}
...
function ae() {
  ...
  return s.prototype.loadGeometry = function() {
    ...
    for (var i = this.geometry, u = 1 / 0, l = -1 / 0, c = 1 / 0, p = -1 / 0, m = 0; m < i.length; m++)
      for (var g = i[m], y = 0; y < g.length; y++) {
        var o = g[y];
...
function le() {
  ...
  function u(o, f) {
    ...
    for (d = 0; d < x.length; d++)
      f.writeStringField(3, x[d]);
    var S = v.values;
    for (d = 0; d < S.length; d++)
      f.writeMessage(4, y, S[d]);
  }
  function l(o, f) {
    var d = o.feature;
  ...
  function c(o, f) {
    var d = o.feature, v = o.keys, x = o.values, S = o.keycache, _ = o.valuecache;
    for (var F in d.properties) {
      var w = d.properties[F], k = S[F];
      if (w !== null) {
        typeof k > "u" && (v.push(F), k = v.length - 1, S[F] = k), f.writeVarint(k);
        var T = typeof w;
        T !== "string" && T !== "boolean" && T !== "number" && (w = JSON.stringify(w));
        var M = T + ":" + w, A = _[M];
        typeof A > "u" && (x.push(w), A = x.length - 1, _[M] = A), f.writeVarint(A);
      }
    }
  }
  function p(o, f) {
  ...
  function g(o, f) {
    for (var d = o.loadGeometry(), v = o.type, x = 0, S = 0, _ = d.length, F = 0; F < _; F++) {
      var w = d[F], k = 1;
      v === 1 && (k = w.length), f.writeVarint(p(1, k));
      for (var T = v === 3 ? w.length - 1 : w.length, M = 0; M < T; M++) {
        M === 1 && v !== 1 && f.writeVarint(p(2, T - 1));
        var A = w[M].x - x, L = w[M].y - S;
        f.writeVarint(m(A)), f.writeVarint(m(L)), x += A, S += L;
      }
...
var ue = le();
const Sn = /* @__PURE__ */ te(ue), kn = `var jt = /^-?(?:\\d+(?:\\.\\d*)?|\\.\\d+)(?:e[+-]?\\d+)?$/i, Qe = Math.ceil, re = Math.floor, j = "[BigNumber Error] ", yt = j + "Number primitive has more than 15 significant digits: ", se = 1e14, G = 14, je = 9007199254740991, et = [1, 10, 100, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9, 1e10, 1e11, 1e12, 1e13], ye = 1e7, X = 1e9;
function Dt(t) {
...
`, Pn = typeof self < "u" && self.Blob && new Blob(["URL.revokeObjectURL(import.meta.url);", kn], { type: "text/javascript;charset=utf-8" });
function he(r) {
  let n;
  try {
...
function pe(r, n = {}) {
  ...
  const y = (x) => {
    if (Array.isArray(x)) {
      const S = Number(x[0]), _ = Number(x[1]);
      s.push(Number.isFinite(S) ? S : 0, Number.isFinite(_) ? _ : 0);
    } else if (x && (typeof x.x == "number" || typeof x.y == "number")) {
      const S = Number(x.x), _ = Number(x.y);
      s.push(Number.isFinite(S) ? S : 0, Number.isFinite(_) ? _ : 0);
    } else
      s.push(0, 0);
  };
  for (const x of r) {
    const S = x.id == null ? "" : String(x.id), _ = x.geometry || {}, F = _.type || "Unknown", w = { id: S, type: F, coordsOffset: m, coordsLength: 0 };
    if (F === "Point") {
      const M = _.coordinates || [];
      y(M), w.coordsLength = 2;
    } else if (F === "LineString" || F === "MultiPoint") {
      const M = _.coordinates || [];
      for (const A of M) y(A);
      ...
      for (const A of M) {
        ...
        for (const L of A) y(L);
      }
      w.coordsLength = w.ringLengths.reduce((A, L) => A + L, 0) * 2;
      ...
      for (const A of M) {
        ...
        for (const L of A) {
          w.ringLengths.push(L.length || 0);
          for (const B of L) y(B);
        }
      }
      w.coordsLength = w.ringLengths.reduce((A, L) => A + L, 0) * 2;
    } else
      w.coordsLength = 0;
    const k = x.properties || {}, T = [];
    ...
    w.props = T, m += w.coordsLength, e.push(w);
  }
  let o;
  if (n.propsBuffer)
    ...
    const x = n.pool.rent(g);
    o = new Uint8Array(x, 0, g);
  } else
    o = new Uint8Array(g);
  let f = 0;
  for (const x of i)
    o.set(x, f), f += x.length;
  const d = s.length;
  let v;
  if (n.coordsBuffer)
    ...
    const x = n.pool.rent(d * 4);
    v = new Float32Array(x, 0, d);
  } else
...
function an(r, n, e, s) {
  ...
  for (let c = 0; c < (r.length || 0); c++) {
    const p = r[c] || {}, m = p.id, g = {};
    if (Array.isArray(p.props) && p.props.length && s && s.length)
      for (const x of p.props) {
        const [S, _, F] = x;
        try {
          const w = u.subarray(_, _ + F);
          g[s[S]] = JSON.parse(un.decode(w));
        } catch {
        }
      }
    const y = p.type || "Unknown";
    let o = p.coordsOffset || 0;
    const f = o + (p.coordsLength || 0);
    let d = null;
    if (y === "Point") {
      const x = i[o], S = i[o + 1], _ = Number.isFinite(x) ? Math.max(-180, Math.min(180, x)) : 0, F = Number.isFinite(S) ? Math.max(-90, Math.min(90, S)) : 0;
      if ((!Number.isFinite(x) || !Number.isFinite(S)) && !V) {
        V = !0;
        try {
          console.warn("decodeFeaturesBinary: encountered non-finite coordinate, replacing with safe value", { index: c, id: m, rawX: x, rawY: S });
        } catch {
        }
      }
      d = { type: "Point", coordinates: [_, F] };
    } else if (y === "LineString" || y === "MultiPoint") {
      const x = [];
      for (; o < f; o += 2) {
        const S = i[o], _ = i[o + 1], F = Number.isFinite(S) ? Math.max(-180, Math.min(180, S)) : 0, w = Number.isFinite(_) ? Math.max(-90, Math.min(90, _)) : 0;
        if ((!Number.isFinite(S) || !Number.isFinite(_)) && !V) {
          V = !0;
          try {
            console.warn("decodeFeaturesBinary: encountered non-finite coordinate in linestring/multipoint, replacing with safe value", { index: c, id: m, rawX: S, rawY: _ });
          } catch {
          }
        }
        x.push([F, w]);
      }
      d = { type: y, coordinates: x };
    } else if (y === "Polygon") {
      const x = [], S = p.ringLengths || [];
      for (const _ of S) {
        const F = [];
        for (let w = 0; w < _; w++) {
          const k = i[o], T = i[o + 1], M = Number.isFinite(k) ? Math.max(-180, Math.min(180, k)) : 0, A = Number.isFinite(T) ? Math.max(-90, Math.min(90, T)) : 0;
          if ((!Number.isFinite(k) || !Number.isFinite(T)) && !V) {
            V = !0;
            try {
              console.warn("decodeFeaturesBinary: encountered non-finite coordinate in polygon, replacing with safe value", { index: c, id: m, rawX: k, rawY: T });
            } catch {
            }
          }
          F.push([M, A]), o += 2;
      ...
      d = { type: "Polygon", coordinates: x };
    } else if (y === "MultiPolygon") {
      const x = [], S = p.polygonRingCounts || [], _ = p.ringLengths || [];
      let F = 0;
      for (const w of S) {
        const k = [];
        for (let T = 0; T < w; T++) {
          ...
          for (let L = 0; L < M; L++) {
            ...
            if ((!Number.isFinite(B) || !Number.isFinite(O)) && !V) {
              V = !0;
              try {
                console.warn("decodeFeaturesBinary: encountered non-finite coordinate in multipolygon, replacing with safe value", { index: c, id: m, rawX: B, rawY: O });
              } catch {
              }
            }
            A.push([t, a]), o += 2;
      ...
      d = { type: "MultiPolygon", coordinates: x };
    } else if (o < f) {
      const x = i[o], S = i[o + 1], _ = Number.isFinite(x) ? Math.max(-180, Math.min(180, x)) : 0, F = Number.isFinite(S) ? Math.max(-90, Math.min(90, S)) : 0;
      if ((!Number.isFinite(x) || !Number.isFinite(S)) && !V) {
        V = !0;
        try {
          console.warn("decodeFeaturesBinary: encountered non-finite coordinate in fallback path, replacing with safe value", { index: c, id: m, rawX: x, rawY: S });
        } catch {
        }
      }
      d = { type: "Point", coordinates: [_, F] };
    ...
    l.push({ type: "Feature", id: m, geometry: d, properties: v });
  }
  return l;
}
const de = new ArrayBuffer(4), _n = new DataView(de);
function ge() {
  return 2166136261;
}
function D(r, n) {
...
function R(r, n) {
  const e = Number(n) || 0;
  return _n.setFloat32(0, e, !0), r = D(r, _n.getUint32(0, !0)), r;
}
function ye(r, n) {
  if (!n) return r;
...
class me {
          ...
          try {
            ...
            if (s.updateDiffs && Array.isArray(s.updateDiffs))
              ...
              try {
                ...
                for (const x of s.updateDiffsMeta) {
                  const S = { id: x.id };
                  if (x.removeAllProperties && (S.removeAllProperties = !0), Array.isArray(x.removeProperties) && x.removeProperties.length && (S.removeProperties = x.removeProperties.map((_) => o[_])), Array.isArray(x.addOrUpdate) && x.addOrUpdate.length) {
                    const _ = [];
                    for (const F of x.addOrUpdate) {
                      const [w, k, T] = F, M = o[w];
                      try {
                        const A = d.subarray(k, k + T), L = JSON.parse(v.decode(A));
                        _.push({ key: M, value: L });
                      } catch {
                      }
                    }
                    _.length && (S.addOrUpdateProperties = _);
                  }
                  p.push(S);
                }
                try {
            ...
            const m = new Map((c || []).map((o) => [String(o.id), o])), g = p.map((o) => {
              const f = { id: o.id }, d = m.get(String(o.id));
            ...
            if (u ? y.removeAll = !0 : i.length && (y.remove = i), l.length && (y.add = l), g.length && (y.update = g), this.gjsource && typeof this.gjsource.updateData == "function")
              try {
                this.gjsource.updateData(y);
                try {
                  if (l && l.length) {
                    for (const o of l)
                      if (o && o.id != null)
          ...
          try {
            if (this._pendingPost) {
              ...
              if (this._lastGeomHashes && this._lastGeomHashes.size === c.length) {
                for (const [g, y] of p.entries())
                  if (this._lastGeomHashes.get(g) !== y) {
                    m = !1;
                    break;
                  }
              } else
              ...
              try {
                const { meta: g, keys: y, propsBuffer: o, coordsArray: f } = pe(this._pendingPost.features || [], { pool: this._abPool, useSharedKeyTable: !0 }), d = Object.fromEntries(p);
                this.minion.postMessage({ type: "features_bin", meta: g, keys: y, propsBuf: o.buffer, tolerance: this._pendingPost.tolerance, coords: f.buffer, cacheSize: this.cacheSize, promoteId: this.fid, hashes: d }, [o.buffer, f.buffer]), this._lastGeomHashes = p;
              } catch {
                try {
                  const y = Object.assign({}, this._pendingPost, { promoteId: this.fid }), o = JSON.stringify(y), f = ln.encode(o);
  ...
  _protocol = async (n) => {
    ...
    let l;
    if (u.status === 200) {
      const c = i.length, [p, m, g] = i.slice(c - 4, c - 1).map((d) => d * 1), y = await u.arrayBuffer(), o = new ne(new Tn(y)), f = {
        layers: Object.entries(o.layers).reduce((d, [v, x]) => ({
          ...d,
          [v]: {
            ...x,
            feature: (S) => {
              const _ = x.feature(S), w = _.loadGeometry().flat(1 / 0).some(
                (k) => k.x >= x.extent - 1 || k.y >= x.extent - 1 || k.x <= 1 || k.y <= 1
              );
              return _.properties.clipped = w, _;
            }
          }
        }), {})
      };
...
maplibregl.VectorTileSource.prototype.ProperLabels = function(r) {
  ...
  return this._proper || (this._proper = new me(n)), this._proper;
};
export {
  me as default
};
