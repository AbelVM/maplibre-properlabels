let k, x;
function H() {
  return k !== void 0 ? k === !1 ? null : k : typeof TextEncoder < "u" ? (k = new TextEncoder(), k) : typeof Buffer < "u" && typeof Buffer.from == "function" ? (k = { encode: (h) => new Uint8Array(Buffer.from(h)) }, k) : (k = !1, null);
}
function K() {
...
class R {
  ...
  constructor({
    ...
    for (let d = 0; d < Math.min(l || 0, this.maxPoolSize); d++)
...
function S(h, e, n = void 0) {
  ...
  if (Array.isArray(h)) {
    ...
    for (let s = 0; s < h.length; s++) if (!S(h[s], e[s], n)) return !1;
    return !0;
  }
  ...
  for (let s = 0; s < o.length; s++) {
    const l = o[s];
    if (!Object.prototype.hasOwnProperty.call(e, l) || !S(h[l], e[l], n)) return !1;
  }
  return !0;
}
let E = null;
...
const Z = () => typeof globalThis < "u" && globalThis && globalThis.console ? globalThis.console : typeof self < "u" && self && self.console ? self.console : typeof window < "u" && window && window.console ? window.console : typeof global < "u" && global && global.console ? global.console : null, v = Z();
class q {
  /**
...
const Y = /* @__PURE__ */ Symbol("PowerSubscriberSet.original");
class T {
  /**
   * @param {Object} [options]
...
class U {
  ...
  on(e, n) {
    ...
    t || (t = new T({ maxListeners: this._maxListeners, weak: this._weak }), this._listeners.set(e, t));
    const r = t.add(n);
  ...
  listeners(e) {
    ...
    return n ? n instanceof T ? n.values() : Array.from(n).map((t) => t && typeof t.deref == "function" ? t.deref() : t).filter(Boolean) : [];
  }
  /**
...
class O {
  ...
  constructor(e, n = {}) {
    ...
    const y = d ? Math.min(this.minSize, this.maxSize) : Math.min(Math.max(r, this.minSize), this.maxSize);
    ...
    for (let o = 0; o < e.length; o++) {
      ...
      if (s !== null && typeof s == "object" && !ArrayBuffer.isView(s) && !(s instanceof ArrayBuffer)) {
        ...
        try {
          ...
          if (t) {
            const d = u.slice();
            i[o] = { message: d, transfer: [d.buffer] };
          } else
  ...
  _addWorkerInstance(e) {
    ...
    t.onmessage = (s) => {
      ...
      try {
        ...
        try {
          const d = s && s.data;
          if (d && typeof d.duration == "number" && Number.isFinite(d.duration) ? u = Math.max(0, Number(d.duration)) : c != null && (u = Math.max(0, l - c)), u != null) {
            const f = this._autoScale && this._autoScale.alpha || 0.2;
            ...
            const p = u - this._taskDurationsWelfordMean;
            this._taskDurationsWelfordMean += p * m / this._taskDurationsWelfordCount;
            const g = u - this._taskDurationsWelfordMean;
            this._taskDurationsWelfordM2 += p * g, u < this._taskDurationsMin && (this._taskDurationsMin = u), u > this._taskDurationsMax && (this._taskDurationsMax = u);
          }
        } catch (d) {
          this._debugLog && this._debugLog(d, "worker.onmessage: latency tracking inner");
        }
      } catch (c) {
  ...
  postMessage(e, n, t) {
    ...
    if (o) {
      ...
      e = Object.assign({}, e, { correlationId: a }), s = new Promise((d, f) => {
        const m = { resolve: d, reject: f, timer: null }, y = a != null ? String(a) : a;
        t && t.timeout && (m.timer = setTimeout(() => {
          try {
            ...
            try {
              f(new Error("postMessage response timeout"));
            } catch {
            }
          }
    ...
    if (i && i.tasks < this._maxTasksPerWorker)
      try {
        const u = w(), d = this._prepareForTransfer(e, n, t);
        return d.transfer && d.transfer.length ? i.worker.postMessage(d.message, d.transfer) : i.worker.postMessage(d.message), i._startTimes && typeof i._startTimes.push == "function" && i._startTimes.push(u), i.tasks++, this._activeTasks++, i.lastActive = u, this._updateIdleState(), o ? s : !0;
      } catch (u) {
        if (o && a) {
          try {
    ...
    if (r == null && this.workers.length < this.maxSize) {
      ...
      try {
        const d = w(), f = this._prepareForTransfer(e, n, t);
        return f.transfer && f.transfer.length ? u.worker.postMessage(f.message, f.transfer) : u.worker.postMessage(f.message), u._startTimes && typeof u._startTimes.push == "function" && u._startTimes.push(d), u.tasks++, this._activeTasks++, u.lastActive = d, this._updateIdleState(), o ? s : !0;
      } catch (d) {
        if (o && a) {
          try {
          ...
          return this._logger.error(d, "Failed to postMessage to new worker"), s;
        }
        return this._logger.error(d, "Failed to postMessage to new worker"), !1;
      }
    }
    if (this.taskQueueEnabled) {
      const u = this._prepareForTransfer(e, n, t), d = this._queuePolicy;
      if (d === "reject")
        return o && a ? (this._cleanupPendingResponse(a, {
          rejectWith: new Error("postMessage rejected by queue policy")
        }), s) : !1;
      if (d === "drop-newest" && this.queue.length > 0)
        return o && a ? (this._cleanupPendingResponse(a, {
          rejectWith: new Error("postMessage rejected by queue policy")
        }), s) : !1;
      if (d === "drop-oldest" && this.queue.length > 0) {
        const m = this.queue.shift();
        m && m.correlationId != null && this._cleanupPendingResponse(m.correlationId, {
      ...
      o && a && (f.correlationId = a), this.queue.push(f);
      try {
        if (Number.isFinite(this._queueHighThreshold) && this.queue.length > this._queueHighThreshold && !this._queueHighCrossed) {
          ...
          try {
            ...
            this._logger.error(m, "pool queue high listener error");
          }
        }
      } catch {
      }
    ...
    try {
      const u = w(), d = this._prepareForTransfer(e, n);
      return d.transfer && d.transfer.length ? c.worker.postMessage(d.message, d.transfer) : c.worker.postMessage(d.message), c._startTimes && typeof c._startTimes.push == "function" && c._startTimes.push(u), c.tasks++, this._activeTasks++, c.lastActive = u, this._updateIdleState(), o ? s : !0;
    } catch (u) {
      if (o && a) {
        try {
          this._cleanupPendingResponse(a, { rejectWith: u });
  ...
  postMessageBatch(e, n) {
    ...
    if (t) {
      ...
      for (let d = 0; d < e.length; d++) {
        const f = e[d] || {}, m = Object.assign({}, n);
        r && (m.correlationId = String(r(d, f))), u[d] = this.postMessage(f.message, f.transfer, m);
      }
      return u;
    }
    ...
    for (let u = 0; u < e.length; u++) {
      const d = e[u] || {}, f = s[u] || { message: d.message, transfer: d.transfer };
      let m = !1;
      ...
      if (!y && a == null && (y = this._findLeastLoadedWorker()), y && y.tasks < this._maxTasksPerWorker)
        try {
          const p = w();
          f.transfer && f.transfer.length ? y.worker.postMessage(f.message, f.transfer) : y.worker.postMessage(f.message), y._startTimes && typeof y._startTimes.push == "function" && y._startTimes.push(p), y.tasks++, this._activeTasks++, y.lastActive = p, c = !0, i[u] = !0, m = !0, l = y.tasks < this._maxTasksPerWorker ? y : null;
        } catch {
          i[u] = !1, m = !0;
        }
      if (!m && a == null && this.workers.length < this.maxSize)
        try {
          ...
          f.transfer && f.transfer.length ? p.worker.postMessage(f.message, f.transfer) : p.worker.postMessage(f.message), p._startTimes && typeof p._startTimes.push == "function" && p._startTimes.push(g), p.tasks++, this._activeTasks++, p.lastActive = g, c = !0, i[u] = !0, m = !0, l = p.tasks < this._maxTasksPerWorker ? p : null;
        } catch {
          i[u] = !1, m = !0;
        }
      if (!m) {
        ...
        if (this.taskQueueEnabled) {
          ...
          p === "reject" || p === "drop-newest" && this.queue.length > 0 ? i[u] = !1 : (p === "drop-oldest" && this.queue.length > 0 && this.queue.shift(), o.push({ message: f.message, transfer: f.transfer }), i[u] = !0);
          ...
          try {
            const _ = w();
            f.transfer && f.transfer.length ? g.worker.postMessage(f.message, f.transfer) : g.worker.postMessage(f.message), g._startTimes && typeof g._startTimes.push == "function" && g._startTimes.push(_), g.tasks++, this._activeTasks++, g.lastActive = _, c = !0, i[u] = !0;
          } catch (_) {
            i[u] = !1, this._logger.error(_, "Failed to postMessage to fallback worker");
          }
        }
      }
    }
  ...
  _autoScaleTick() {
    try {
      ...
      if (l || c) {
        if (a < this.maxSize)
          try {
            const f = Math.min(this.maxSize - a, n.stepUp || 1);
            for (let m = 0; m < f; m++) this._addWorkerInstance();
            ...
            this._debugLog && this._debugLog(f, "autoScale: addWorker failed");
          }
        return;
      }
      ...
      if ((o != null ? o < u : !1) && this.queue.length === 0 && a > this.minSize)
        try {
          const f = Math.min(a - this.minSize, n.stepDown || 1);
          for (let m = 0; m < f; m++) {
          ...
          this._debugLog && this._debugLog(f, "autoScale: remove worker failed");
        }
    } catch (e) {
  ...
  getStats() {
    ...
    let d = 0, f = 0, m = 0, y = 0, p = 0;
    const g = this._taskDurationsWelfordCount || 0;
    if (g > 0) {
      d = this._taskDurationsMin === Number.POSITIVE_INFINITY ? 0 : this._taskDurationsMin, f = this._taskDurationsMax === Number.NEGATIVE_INFINITY ? 0 : this._taskDurationsMax, m = this._taskDurationsWelfordMean;
    ...
    return {
      ...
      performance: {
        ...
        timePerTask: { max: f, min: d, average: m, stddev: y },
        percentSlowTasks: p
      },
...
class W {
  ...
  constructor(e, n = {}) {
    ...
    this._flushFn = e, this._scheduling = n.scheduling === "macrotask" ? "macrotask" : "microtask", this._scheduled = !1, this._timer = null;
  }
  ...
  schedule() {
    if (!this._scheduled) {
      if (this._scheduled = !0, this._scheduling === "macrotask") {
...
class te {
  ...
  constructor({
    ...
    this.map = e, this.source = n, this.sourceLayer = t, this.fid = r, this.tileSize = i, this.tolerance = o, this.units = s, this.gatherPoolSize = Math.max(1, Number.isFinite(Number(f)) ? Math.floor(Number(f)) : 1), this.postDelay = Number.isFinite(Number(l)) ? Math.max(0, Number(l)) : 0;
    const M = c ?? u;
   ...
   */
  handleSourceData(e) {
    if (this._disposed || !e || e.sourceId !== this.source.id) return;
    if (typeof e.isSourceLoaded == "boolean") {
      const g = this._sourceLoaded;
      this._sourceLoaded = e.isSourceLoaded, !g && this._sourceLoaded && this._changedGroups.size > 0 && this._scheduleGather();
    }
    ...
    if (s) {
      l = `map|z${n.z}`;
      const g = Date.now();
      if (this._lastMapFallbackUnique === l && g - this._lastMapFallbackAt < this.mapFallbackCooldown)
        return;
      this._lastMapFallbackUnique = l, this._lastMapFallbackAt = g;
    }
    const c = o ? o(i) : a ? a(i) : null;
    if (!Array.isArray(c) || (r = c, !r.length)) return;
    const u = this._computeTileFingerprint(r), d = this._tileFingerprints.get(l), f = d != null && d === u, m = this.piecesCache.has(l);
    if (this._pendingTiles.has(l) && f || m && f) return;
  ...
  _countCoordinates(e) {
    ...
    let n = 0;
    if (Array.isArray(e[0]) && Array.isArray(e[0][0])) {
      for (const t of e)
        n += this._countCoordinates(t);
      return n;
    }
    return e.length;
  }
  ...
  _removeTileGroups(e) {
    ...
    if (n) {
      for (const t of n) {
        const r = this._groupToTiles.get(t);
        r && (r.delete(e), r.size === 0 && this._groupToTiles.delete(t));
      }
      this._tileGroups.delete(e);
    }
  }
  _serializePropertyValue(e) {
    return e == null || typeof e == "string" || typeof e == "number" || typeof e == "boolean" ? String(e) : Array.isArray(e) ? `array:${e.length}` : typeof e == "object" ? `object:${Object.keys(e).length}` : String(e);
  }
  /**
  ...
  dispose() {
    ...
    this.piecesCache.clear(), this.labelsCache.clear(), this._tileFingerprints.clear(), this._pendingTiles.clear(), this._tilePendingMeta.clear(), this._tileQueue.clear(), this._changedGroups.clear(), this._tileGroups.clear(), this._groupToTiles.clear(), this._bus.clear(), this.gjSource = null;
  }
  _schedulePendingTileTimeoutCheck() {
    if (this._tileTimeoutHandle || this._tilePendingMeta.size === 0 || this.tileTimeout <= 0) return;
    const e = Math.max(25, Math.min(1e3, Math.floor(this.tileTimeout / 2) || 250));
    this._tileTimeoutHandle = setTimeout(() => {
      this._tileTimeoutHandle = null, this._expirePendingTiles(!1);
    }, e);
  }
  _expirePendingTiles(e = !1) {
    if (this._disposed || this._tilePendingMeta.size === 0) return;
    const n = Date.now();
    let t = !1;
    for (const [r, i] of this._tilePendingMeta.entries())
      !e && i.deadline > n || (i.retries < this.tileMaxRetries ? (i.retries += 1, i.deadline = n + Math.max(1, this.tileTimeout), i.correlationId = `${r}:${++this._tileCorrelationSeq}`, i.payload = {
        ...i.payload,
        correlationId: i.correlationId
      }, this._tileQueue.push(i.payload), t = !0) : (this._tilePendingMeta.delete(r), this._pendingTiles.delete(r)));
    t && this._scheduleTileDrain(), this._tilePendingMeta.size > 0 && this._schedulePendingTileTimeoutCheck();
  }
  /**
   ...
   */
  _scheduleTileDrain() {
    if (!(this._tileDrainScheduled || this._tileDrainTimeout)) {
      if (this._tileDrainScheduled = !0, this.postDelay > 0) {
  ...
  _drainTileQueue() {
    const e = [];
    let n;
    const t = Number.isFinite(Number(this._maxTileBatch)) ? Math.max(1, Math.floor(Number(this._maxTileBatch))) : 128;
    let r = 0;
    for (; r < t && (n = this._tileQueue.shift()) !== void 0; )
      e.push({ message: n }), r += 1;
    if (e.length > 0) {
      try {
        this.tilePool.postMessageBatch(e, { zeroCopy: !0 });
      } catch {
        try {
          this._expirePendingTiles(!0);
        } catch {
        }
      }
      r >= t && this._scheduleTileDrain();
    }
  }
  /**
  ...
  _dispatchGather() {
    if (this._disposed) return;
    const e = Number.isFinite(this._activeZoom) ? this._activeZoom : Math.floor(this.map.getZoom());
    let n;
    if (this._changedGroups.size > 0) {
      const r = /* @__PURE__ */ new Set();
      for (const i of this._changedGroups) {
        const o = this._groupToTiles.get(i);
        if (o)
          for (const a of o)
            this._getTileZoom(a) === e && r.add(a);
      }
      if (r.size === 0) {
      ...
      n = {};
      for (const i of r) {
        const o = this.piecesCache.get(i);
        o && (n[i] = o);
      }
      this._changedGroups.clear();
    } else {
      const r = Array.from(this.piecesCache.entries("LRU"));
      if (!r.length) return;
      const i = Number.isFinite(Number(this._maxGatherPieces)) ? Math.max(1, Math.floor(Number(this._maxGatherPieces))) : 200, o = [];
      for (let a = 0; a < r.length && o.length < i; a += 1) {
        const [s, l] = r[a];
        this._getTileZoom(s) === e && o.push([s, l]);
      }
      if (o.length === 0) return;
      n = Object.fromEntries(o);
    }
    if (!n || !Object.keys(n).length) return;
    const t = ++this._gatherRound;
    this._scheduleGatherTimeout(t);
    try {
  ...
  _scheduleGatherTimeout(e) {
    if (this.gatherTimeout <= 0) return;
    const n = setTimeout(() => {
      this._pendingGatherRounds.has(e) && (this._pendingGatherRounds.delete(e), this._scheduleDiffFlush({ gatherRound: e, timestamp: Date.now() }));
    }, this.gatherTimeout);
    this._pendingGatherRounds.set(e, n);
  }
  _clearGatherTimeout(e) {
    const n = this._pendingGatherRounds.get(e);
    n && (clearTimeout(n), this._pendingGatherRounds.delete(e));
  }
  /**
  ...
  _onGatherMessage(e) {
    ...
    if (n) {
      if (n.type === "commit") {
        const t = Number.isFinite(Number(n.gatherRound)) ? Number(n.gatherRound) : 0, r = Number.isFinite(Number(n.timestamp)) ? Number(n.timestamp) : Date.now();
        t && this._clearGatherTimeout(t), this._bus.emit("commit", { gatherRound: t, timestamp: r });
        return;
      }
  ...
  _keepNonContained = (e) => {
    ...
    const n = e.map((s) => typeof s == "string" ? s.split("-") : []), t = n.length;
    if (t <= 1) return e.slice();
    const r = /* @__PURE__ */ new Map();
    for (let s = 0; s < t; s += 1) {
      const l = n[s], c = /* @__PURE__ */ new Set();
      for (let u = 0; u < l.length; u += 1) {
        const d = l[u];
        if (c.has(d)) continue;
        c.add(d);
        const f = r.get(d);
        f ? f.add(s) : r.set(d, /* @__PURE__ */ new Set([s]));
      }
    }
    const i = new Array(t);
    for (let s = 0; s < t; s += 1) {
      const l = /* @__PURE__ */ new Map(), c = n[s];
      for (let u = 0; u < c.length; u += 1) {
        const d = c[u], f = l.get(d);
        f ? f.push(u) : l.set(d, [u]);
      }
      i[s] = l;
    }
    const o = (s, l) => {
   ...
   */
  _collectLabelDiff(e) {
    const n = Number.isFinite(Number(e.gatherRound)) ? Number(e.gatherRound) : 0, t = Number.isFinite(Number(e.timestamp)) ? Number(e.timestamp) : 0;
    if (n && n < this._lastGatherRound) return;
   ...
   * @returns {void}
   */
  _scheduleDiffFlush({ gatherRound: e = 0, timestamp: n = 0 } = {}) {
    if (e && e > this._lastGatherRound && (this._lastGatherRound = e, this._lastGatherTimestamp = 0), n && n > this._lastGatherTimestamp && (this._lastGatherTimestamp = n), !(!this._diffFlushInProgress && !this._diffScheduled && this._diffAdd.size === 0 && this._diffRemove.size === 0) && !(!this._diffFlushInProgress && !this._diffScheduled && e && e < this._lastGatherRound)) {
      if (this._diffFlushInProgress) {
        if (e && e <= this._currentFlushGatherRound || n && n <= this._currentFlushTimestamp)
          return;
        this._diffFlushQueued = !0, e && e > this._diffFlushQueuedGatherRound && (this._diffFlushQueuedGatherRound = e), n && n > this._diffFlushQueuedTimestamp && (this._diffFlushQueuedTimestamp = n);
        return;
      }
      if (this._diffScheduled) {
        e && e > this._diffFlushQueuedGatherRound && (this._diffFlushQueuedGatherRound = e), n && n > this._diffFlushQueuedTimestamp && (this._diffFlushQueuedTimestamp = n);
        return;
      }
      this._diffScheduled = !0, this._diffScheduler.schedule();
    }
  }
  async _runDiffFlush() {
    if (!this._diffScheduled || (this._diffScheduled = !1, this._diffFlushInProgress)) return;
    this._diffFlushInProgress = !0;
    const e = Math.max(this._lastGatherRound, this._diffFlushQueuedGatherRound), n = Math.max(this._lastGatherTimestamp, this._diffFlushQueuedTimestamp, Date.now());
    this._currentFlushGatherRound = e, this._currentFlushTimestamp = n;
    const t = this._diffFlushQueuedGatherRound, r = this._diffFlushQueuedTimestamp;
    this._diffFlushQueuedGatherRound = 0, this._diffFlushQueuedTimestamp = 0;
    let i = !0;
    try {
      i = await this._flushDiffs();
    } finally {
      this._diffFlushInProgress = !1, this._currentFlushGatherRound = 0, this._currentFlushTimestamp = 0, i || this._scheduleDiffRetry(), this._diffFlushQueued && (this._diffFlushQueued = !1, this._scheduleDiffFlush({ gatherRound: t, timestamp: r }));
    }
  }
  _scheduleDiffRetry() {
    if (this._disposed || this._diffRetryHandle || this._diffAdd.size === 0 && this._diffRemove.size === 0) return;
    const e = Math.min(1e3, Math.max(25, 25 * Math.pow(2, this._diffRetryCount)));
    this._diffRetryCount += 1, this._diffRetryHandle = setTimeout(() => {
      this._diffRetryHandle = null, this._scheduleDiffFlush({ gatherRound: this._lastGatherRound, timestamp: Date.now() });
    }, e);
  }
  /**
   ...
   */
  async _flushDiffs() {
    if (this._disposed || !this.gjSource || this._diffAdd.size === 0 && this._diffRemove.size === 0) return !0;
    const e = this._diffAdd.size > 0 ? [...this._diffAdd.entries()] : [];
    let n = [];
    if (e.length === 1)
      n = [e[0][1]];
    else if (e.length > 1) {
...
class B {
  ...
  constructor(e) {
    ...
    if (typeof e.sourceLayer != "string" || !e.sourceLayer.trim())
      throw new Error("ProperLabels requires a valid non-empty sourceLayer string.");
    if (e.fid != null && typeof e.fid != "string")
      throw new Error("ProperLabels expects fid to be a string.");
    if (e.units != null && e.units !== "meters" && e.units !== "m")
      throw new Error('ProperLabels expects units to be either "meters" or "m".');
    if (e.postDelay != null) {
      const r = Number(e.postDelay);
      if (!Number.isFinite(r) || r < 0)
        throw new Error("ProperLabels expects postDelay to be a non-negative number.");
    }
    this.map = e.map;
    const n = e.source instanceof maplibregl.VectorTileSource ? e.source : this.map.getSource(e.source);
    if (!n)
      throw new Error(
        `ProperLabels could not resolve source ${String(e.source)}. Provide a valid source id or a VectorTileSource instance attached to the map.`
      );
    if (!(n instanceof maplibregl.VectorTileSource))
      throw new Error("ProperLabels source must be a MapLibre VectorTileSource.");
    const t = this.map.getSource(n.id);
    if (!t || t !== n)
      throw new Error(
   ...
   */
  dispose() {
    if (this.manager && this.manager.dispose(), this.map && typeof this.map.off == "function" && this._onSourceData && this.map.off("sourcedata", this._onSourceData), !this.keepSource && this.map && typeof this.map.getSource == "function" && typeof this.map.removeSource == "function") {
      const e = `${this.source.id}-proper`;
