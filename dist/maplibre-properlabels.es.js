...
`, v = typeof self < "u" && self.Blob && new Blob(["URL.revokeObjectURL(import.meta.url);", A], { type: "text/javascript;charset=utf-8" });
function I(u) {
  let n;
  try {
...
const P = `var nt = /^-?(?:\\d+(?:\\.\\d*)?|\\.\\d+)(?:e[+-]?\\d+)?$/i, Re = Math.ceil, H = Math.floor, V = "[BigNumber Error] ", qe = V + "Number primitive has more than 15 significant digits: ", Z = 1e14, O = 14, Me = 9007199254740991, Oe = [1, 10, 100, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9, 1e10, 1e11, 1e12, 1e13], se = 1e7, U = 1e9;
function Je(i) {
...
`, b = typeof self < "u" && self.Blob && new Blob(["URL.revokeObjectURL(import.meta.url);", P], { type: "text/javascript;charset=utf-8" });
function L(u) {
  let n;
  try {
...
class E {
  ...
  constructor(n, e = {}) {
    const t = typeof navigator < "u" && navigator.hardwareConcurrency || 2, {
      ...
      workerOptions: p = {},
      maxTasksPerWorker: a = 1 / 0,
      idleTimeout: h = 6e4,
      taskQueue: s = !0
    } = e;
    this._workerSource = n, this._workerOptions = p, this._maxTasksPerWorker = a, this.minSize = Math.max(0, i), this.maxSize = Math.max(this.minSize, l), this.idleTimeout = Math.max(0, h), this.taskQueueEnabled = !!s, this.workers = [], this.queue = [], this._listeners = { message: /* @__PURE__ */ new Set(), error: /* @__PURE__ */ new Set(), messageerror: /* @__PURE__ */ new Set(), idle: /* @__PURE__ */ new Set() }, this._onmessage = null, this._onerror = null, this._onidle = null, this._nextIndex = 0, this._isIdle = !0;
    const o = Math.min(Math.max(r, this.minSize), this.maxSize);
    for (let g = 0; g < o; g++) this._addWorkerInstance(g);
  ...
  _addWorkerInstance(n) {
    ...
    return this.workers.push(t), e.onmessage = (r) => {
      if (t.tasks = Math.max(0, t.tasks - 1), t.lastActive = Date.now(), this.queue.length > 0 && t.tasks < this._maxTasksPerWorker) {
        ...
        try {
          i.transfer ? e.postMessage(i.message, i.transfer) : e.postMessage(i.message), t.tasks++;
        } catch (l) {
      ...
      if (this._onmessage)
        try {
          ...
          console.error("Pool onmessage handler error", i);
        }
      for (const i of this._listeners.message)
        try {
          i(r);
        } catch (l) {
      ...
      if (this._onerror)
        try {
          ...
          console.error("Pool onerror handler error", i);
        }
      for (const i of this._listeners.error)
        try {
          i(r);
        } catch (l) {
      ...
      for (const i of this._listeners.messageerror)
        try {
          i(r);
        } catch (l) {
  ...
  postMessage(n, e) {
    ...
    if (t && t.tasks < this._maxTasksPerWorker)
      try {
        ...
        return console.error("Failed to postMessage to worker", i), !1;
      }
    if (this.workers.length < this.maxSize) {
      const i = this.workers.length, l = this._addWorkerInstance(i);
      try {
        return e ? l.worker.postMessage(n, e) : l.worker.postMessage(n), l.tasks++, l.lastActive = Date.now(), this._updateIdleState(), !0;
      } catch (p) {
        return console.error("Failed to postMessage to new worker", p), !1;
      }
    }
    ...
    try {
      ...
      return console.error("Failed to postMessage to fallback worker", i), !1;
    }
  }
  /**
   ...
   */
  _updateIdleState() {
    const e = this.workers.length > 0 && this.workers.every((i) => i.tasks === 0), t = this.queue.length === 0, r = e && t;
  ...
  addEventListener(n, e) {
    if (n in this._listeners && (this._listeners[n].add(e), n === "idle")) {
      const r = this.workers.length > 0 && this.workers.every((l) => l.tasks === 0), i = this.queue.length === 0;
      if (r && i) {
        const l = { data: { type: "pool:idle", stats: this.getStats() } };
        try {
          e(l);
        } catch (p) {
          console.error("pool idle listener error", p);
        }
      }
    }
  }
  ...
  set onidle(n) {
    if (this._onidle = n, typeof n == "function") {
      const t = this.workers.length > 0 && this.workers.every((i) => i.tasks === 0), r = this.queue.length === 0;
      if (t && r) {
        const i = { data: { type: "pool:idle", stats: this.getStats() } };
        try {
          n(i);
        } catch (l) {
          console.error("Pool onidle handler error", l);
        }
      }
    }
  }
...
class _ {
   ...
   */
  constructor({ maxEntries: n = 1 / 0, maxWeight: e = 1 / 0, weightFn: t = () => 1, defaultTTL: r = 6e4, maxPoolSize: i = 1e3, rejectOversized: l = !1, onEvict: p = null, onExpire: a = null, initialPoolSize: h = 0, maxCleanupPerTick: s = 100 } = {}) {
    this.maxEntries = n, this.maxWeight = e, this.weightFn = t, this.defaultTTL = r, this.maxPoolSize = i, this.rejectOversized = !!l, this.onEvict = typeof p == "function" ? p : null, this.onExpire = typeof a == "function" ? a : null, this.maxCleanupPerTick = Number.isFinite(+s) ? Math.max(1, +s) : 100, this.map = /* @__PURE__ */ new Map(), this.head = null, this.tail = null, this.pool = [];
    for (let o = 0; o < Math.min(h || 0, this.maxPoolSize); o++) this.pool.push({ key: null, value: null, weight: 0, expiresAt: 0, prev: null, next: null });
   ...
   */
  _allocNode(n, e, t, r) {
    const i = this.pool.pop() || { key: null, value: null, weight: 0, expiresAt: 0, prev: null, next: null };
    return i.key = n, i.value = e, i.weight = t || 0, i.expiresAt = r || 0, i.prev = null, i.next = null, i;
  }
  /**
  ...
  set(n, e, { ttl: t = this.defaultTTL, weight: r = null } = {}) {
    const i = Date.now(), l = t == null || t === 1 / 0 ? 0 : i + t, p = r ?? (this.weightFn(e) || 0), a = Number.isFinite(+p) ? Math.max(0, +p) : 0;
    ...
    if (this.map.has(n)) {
      ...
      this.currentWeight -= h.weight || 0, h.value = e, h.weight = a, h.expiresAt = l, this.currentWeight += h.weight || 0, this._moveToTail(h);
    } else {
      const h = this._allocNode(n, e, a, l);
      this.map.set(n, h), this._append(h), this.currentWeight += h.weight || 0, this._evictIfNeeded();
    }
    return this;
  }
  /**
  ...
  get(n) {
    ...
    if (e.expiresAt && e.expiresAt <= t) {
      const r = e.key, i = e.value, l = e.next;
  ...
  hasEqual(n, e, { ignoreExpiry: t = !1 } = {}) {
    ...
    if (typeof i !== "object" || i === null || typeof e !== "object" || e === null)
      return i === e;
    const a = /* @__PURE__ */ new WeakMap(), h = (s, o) => {
      if (s === o) return !0;
      if (s == null || o == null || typeof s !== "object" || typeof o !== "object") return s === o;
      const w = a.get(s);
      if (w && w.has(o)) return !0;
      if (w || a.set(s, /* @__PURE__ */ new WeakSet()), a.get(s).add(o), Object.getPrototypeOf(s) !== Object.getPrototypeOf(o)) return !1;
      if (typeof Uint8Array < "u" && s instanceof Uint8Array) {
        if (!(o instanceof Uint8Array) || s.length !== o.length) return !1;
        for (let c = 0; c < s.length; c++) if (s[c] !== o[c]) return !1;
        return !0;
      }
      if (Array.isArray(s)) {
        if (!Array.isArray(o) || s.length !== o.length) return !1;
        for (let c = 0; c < s.length; c++) if (!h(s[c], o[c])) return !1;
        return !0;
      }
      if (ArrayBuffer.isView(s)) {
        if (!ArrayBuffer.isView(o) || s.byteLength !== o.byteLength) return !1;
        const c = new Uint8Array(s.buffer, s.byteOffset || 0, s.byteLength), f = new Uint8Array(o.buffer, o.byteOffset || 0, o.byteLength);
        for (let y = 0; y < c.length; y++) if (c[y] !== f[y]) return !1;
        return !0;
      }
      if (s instanceof ArrayBuffer) {
        if (!(o instanceof ArrayBuffer) || s.byteLength !== o.byteLength) return !1;
        const c = new Uint8Array(s), f = new Uint8Array(o);
        for (let y = 0; y < c.length; y++) if (c[y] !== f[y]) return !1;
        return !0;
      }
      if (s instanceof Date)
        return o instanceof Date ? s.getTime() === o.getTime() : !1;
      if (s instanceof RegExp)
        return o instanceof RegExp ? s.toString() === o.toString() : !1;
      if (s instanceof Map) {
        if (!(o instanceof Map) || s.size !== o.size) return !1;
        for (const [c, f] of s)
          if (!o.has(c) || !h(f, o.get(c))) return !1;
        return !0;
      }
      if (s instanceof Set) {
        if (!(o instanceof Set) || s.size !== o.size) return !1;
        let c = !0;
        for (const f of s)
        ...
        if (c) {
          for (const f of s) if (!o.has(f)) return !1;
          return !0;
        }
        for (const f of s) {
          let y = !1;
      ...
      const d = Object.keys(s), x = Object.keys(o);
      if (d.length !== x.length) return !1;
      for (let c = 0; c < d.length; c++) {
        const f = d[c];
        if (!Object.prototype.hasOwnProperty.call(o, f) || !h(s[f], o[f])) return !1;
      }
      return !0;
    };
    return h(i, e);
  ...
  cleanupExpiredUpTo(n = 1 / 0) {
    ...
    for (; r && t < n; ) {
      ...
      if (r.expiresAt && r.expiresAt <= e) {
        const l = r.key, p = r.value;
        this.map.delete(l), this.currentWeight -= r.weight || 0, this._cleanupCursor === r && (this._cleanupCursor = i || this.head), this._remove(r);
        try {
          this.onExpire && this.onExpire(l, p);
        } catch {
        }
...
const O = typeof TextEncoder < "u", N = typeof TextDecoder < "u", S = O ? new TextEncoder() : null, T = N ? new TextDecoder() : null, k = (u) => {
  ...
  if (S) return S.encode(n);
  if (typeof Buffer < "u" && typeof Buffer.from == "function") return new Uint8Array(Buffer.from(n));
  throw new Error("No TextEncoder available to encode object");
}, R = (u) => {
  let n;
  if (u instanceof Uint8Array) n = u;
  else if (ArrayBuffer.isView(u)) n = new Uint8Array(u.buffer, u.byteOffset, u.byteLength);
  else if (u instanceof ArrayBuffer) n = new Uint8Array(u);
  else if (typeof Buffer < "u" && typeof Buffer.isBuffer == "function" && Buffer.isBuffer(u)) n = new Uint8Array(u);
  else throw new TypeError("Unsupported input to u82o, expected ArrayBuffer/TypedArray/Buffer");
...
class B {
  constructor(n) {
    ...
    const e = new E(I, { size: 6 }), t = new E(L, { size: 4 }), r = new _({
    ...
    e.onmessage = (a) => {
      if (a.data instanceof ArrayBuffer) {
        const h = a.data, s = R(h);
        if (s.type === "simplified" && s.size > 0) {
          const { unique: o, type: g, ...m } = s;
          r.set(o, m);
        }
      }
    };
    const i = new _({
      maxEntries: this.cacheSize,
      maxWeight: this.cacheSize * 5e3,
      weight: (a) => a.features.length || 0
    }), l = { add: /* @__PURE__ */ new Map(), remove: /* @__PURE__ */ new Set() }, p = () => {
      if (l.add.size === 0 && l.remove.size === 0) {
        console.log("No changes to apply, skipping update"), this.scheduler && (clearInterval(this.scheduler), this.scheduler = null);
        return;
      }
      console.log(`Applying diff with ${l.add.size} additions and ${l.remove.size} removals`);
      const a = [...l.add.values()], h = [...l.remove];
      this.gjSource.updateData({ add: a, remove: h }), l.add.clear(), l.remove.clear();
    };
    return t.onmessage = (a) => {
      if (a.data instanceof ArrayBuffer) {
        const h = a.data, s = R(h), { id: o, features: g } = s;
        if (i.has(o) && !i.hasEqual(o, g)) {
          const m = i.get(o);
          [...new Set(m.map((d) => d.properties._index))].forEach((d) => l.remove.add(d)), g.forEach((d) => l.add.set(d.properties._index, d)), i.set(o, g);
        } else
          g.forEach((m) => l.add.set(m.properties._index, m)), i.set(o, g);
      }
    }, t.addEventListener("idle", p), this.map.on("sourcedata", (a) => {
      if (a.sourceId === this.source.id) {
        const { z: h, x: s, y: o } = a.tile.tileID.canonical, g = `${h}|${s}|${o}`;
        if (!r.has(g)) {
          const m = this.tolerance * Math.pow(10, -0.301 * h + 5.19), w = [], d = this.source.type === "vector" ? { sourceLayer: this.sourceLayer } : {};
          a.tile.querySourceFeatures(w, d);
          const x = {
            collection: {
              type: "FeatureCollection",
              features: w.map((f, y) => ({
        ...
        a.isSourceLoaded && e.addEventListener("idle", (m) => {
          const d = { pieces: Object.fromEntries(r.entries()), tolerance: this.tolerance, unit: this.units, tileSize: this.tileSize }, x = k(d).buffer;
          t.postMessage(x, [x]);
        });
      }
...
maplibregl.VectorTileSource.prototype.ProperLabels = function(u) {
  ...
  return this._proper || (this._proper = new B(n)), this._proper;
};
export {
  B as default
};
