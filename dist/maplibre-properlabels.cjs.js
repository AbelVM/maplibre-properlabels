"use strict";var v=typeof document<"u"?document.currentScript:null;const P=`function B(e, n, t = {}) {
...
`,b=typeof self<"u"&&self.Blob&&new Blob(["URL.revokeObjectURL(import.meta.url);",P],{type:"text/javascript;charset=utf-8"});function O(u){let n;try{if(n=b&&(self.URL||self.webkitURL).createObjectURL(b),!n)throw"";const e=new Worker(n,{type:"module",name:u?.name});return e.addEventListener("error",()=>{(self.URL||self.webkitURL).revokeObjectURL(n)}),e}catch{return new Worker("data:text/javascript;charset=utf-8,"+encodeURIComponent(P),{type:"module",name:u?.name})}}const M=`var nt = /^-?(?:\\d+(?:\\.\\d*)?|\\.\\d+)(?:e[+-]?\\d+)?$/i, Re = Math.ceil, H = Math.floor, V = "[BigNumber Error] ", qe = V + "Number primitive has more than 15 significant digits: ", Z = 1e14, O = 14, Me = 9007199254740991, Oe = [1, 10, 100, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9, 1e10, 1e11, 1e12, 1e13], se = 1e7, U = 1e9;
function Je(i) {
