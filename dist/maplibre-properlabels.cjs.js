...
`,N=typeof self<"u"&&self.Blob&&new Blob(["URL.revokeObjectURL(import.meta.url);",O],{type:"text/javascript;charset=utf-8"});function ee(h){let e;try{if(e=N&&(self.URL||self.webkitURL).createObjectURL(N),!e)throw"";const n=new Worker(e,{type:"module",name:h?.name});return n.addEventListener("error",()=>{(self.URL||self.webkitURL).revokeObjectURL(e)}),n}catch{return new Worker("data:text/javascript;charset=utf-8,"+encodeURIComponent(O),{type:"module",name:h?.name})}}const z=`let oe, le;
function st() {
...
`,L=typeof self<"u"&&self.Blob&&new Blob(["URL.revokeObjectURL(import.meta.url);",z],{type:"text/javascript;charset=utf-8"});function ne(h){let e;try{if(e=L&&(self.URL||self.webkitURL).createObjectURL(L),!e)throw"";const n=new Worker(e,{type:"module",name:h?.name});return n.addEventListener("error",()=>{(self.URL||self.webkitURL).revokeObjectURL(e)}),n}catch{return new Worker("data:text/javascript;charset=utf-8,"+encodeURIComponent(z),{type:"module",name:h?.name})}}class W{constructor(e){return this.map=e.map,this.source=e.source instanceof maplibregl.VectorTileSource?e.source:this.map.getSource(e.source),this.sourceLayer=e.sourceLayer,this.fid=e.fid||"id",this.tileSize=this.source.tileSize||512,this.tolerance=e.tolerance||1e-5,this.cacheSize=e.cacheSize||5e3,this.units=e.units||"meters",this.map.addSource(this.source.id+"-proper",{type:"geojson",maxzoom:this.source.maxzoom,promoteId:"_index",data:{}}),this.gjSource=this.map.getSource(this.source.id+"-proper"),this.manager=new X({map:this.map,source:this.source,sourceLayer:this.sourceLayer,fid:this.fid,tileSize:this.tileSize,tolerance:this.tolerance,cacheSize:this.cacheSize,units:this.units,tileWorkerSource:ee,gatherWorkerSource:ne}),this.manager.setGeoJsonSource(this.gjSource),this.map.on("sourcedata",n=>this.manager.handleSourceData(n)),this.map.refreshTiles(this.source.id),this.gjSource}}maplibregl.VectorTileSource.prototype.ProperLabels=function(h){const e=Object.assign({},h,{map:this._map,source:this});return this._proper||(this._proper=new W(e)),this._proper};module.exports=W;
