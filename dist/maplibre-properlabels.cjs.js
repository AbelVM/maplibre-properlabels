...
`,j=typeof self<"u"&&self.Blob&&new Blob(["URL.revokeObjectURL(import.meta.url);",D],{type:"text/javascript;charset=utf-8"});function re(h){let e;try{if(e=j&&(self.URL||self.webkitURL).createObjectURL(j),!e)throw"";const n=new Worker(e,{type:"module",name:h?.name});return n.addEventListener("error",()=>{(self.URL||self.webkitURL).revokeObjectURL(e)}),n}catch{return new Worker("data:text/javascript;charset=utf-8,"+encodeURIComponent(D),{type:"module",name:h?.name})}}const G=`let ue, he;
function Dt() {
